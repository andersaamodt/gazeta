...
assert_file_contains "$ROOT_DIR/cgi/blog-secure-chat-admin-legacy" 'blog_secure_chat_service_request_json' 'admin endpoint prints daemon json responses explicitly'
...
assert_file_contains "$ROOT_DIR/cgi/blog-secure-chat-admin-legacy" 'blog_require_session true' 'admin endpoint requires admin session'
assert_file_contains "$ROOT_DIR/cgi/blog-secure-chat-admin-legacy" 'sinceSeq:$sinceSeq' 'admin endpoint forwards notification cursor for Secure Chat admin polling'
assert_file_contains "$ROOT_DIR/cgi/blog-secure-chat-service.js" 'notificationStatusPayload' 'secure chat service exposes an admin notification status payload'
assert_file_contains "$ROOT_DIR/cgi/blog-secure-chat-service.js" "action === 'notification-status'" 'secure chat admin endpoint supports notification-status polling'
assert_file_contains "$ROOT_DIR/site/static/nav-auth.js" "action: 'notification-status'" 'nav auth polls Secure Chat admin notifications for signed-in admins'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'secure-chat-saved-hint' 'contact page renders a saved Secure Chat history hint before the closed chat button'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/simplex-web-default-chat.js" 'secure-chat-saved-hint' 'shared secure chat renderer renders a saved history hint before the closed chat button'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'flex-direction: column;' 'Secure Chat saved history hint stacks above the Open Chat button'
