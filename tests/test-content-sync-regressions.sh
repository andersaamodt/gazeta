...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'body.blog-post-resize-enabled:has(#blog-page-root):not(:has(.post-single-item))' 'blog index keeps its body width even if resize mode is present'
