...
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" "blog_nostr_list_page_js_version='20260523-optimistic-votes1'" 'list page script cache buster tracks optimistic vote UI'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'data-video-chat-call-label="Call Anders Now"' 'video chat widget owner call button names the direct call action'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'video-chat-widget.js?v=20260524-scheduled-rooms1' 'contact page loads the scheduled-room video widget build'
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" '<script src="/static/video-chat-widget.js?v=20260524-scheduled-rooms1" data-video-chat-widget="1"></script>' 'contact page HTML loads the video widget before contact-page hydration'
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" "blog_nostr_contact_page_js_version='20260523-login-note1'" 'contact page cache buster tracks Secure Chat login prompt rendering'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'function renderVoteTooltip(entry)' 'list vote controls render one shared tooltip'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" "You've " 'list vote tooltip includes total viewer vote count copy'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.list-entry-vote-controls:hover .list-entry-vote-tooltip' 'list vote tooltip hover target covers the whole vote control'
assert_list_vote_cooldown_merge
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'width: 100% !important;' 'final navbar rule matches page content width'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'nav-search-gem-size: 26px;' 'navbar search button matches the compact login-button height'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'conic-gradient(from 145deg at 50% 52%' 'navbar search button uses a faceted gem surface instead of parchment'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'color: var(--theme_green_fg, var(--theme_green, var(--green, #2c8b6e))) !important;' 'navbar search icon uses the green theme color'
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" 'color: var(--malachite, #2c8b6e) !important;' 'Lapidarist navbar search icon uses malachite green'
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" 'nav-search-gem-size: 26px;' 'Lapidarist navbar search button is compact like the login button'
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" 'var(--gold-leaf, #b68b35)' 'Lapidarist navbar search gem uses a contrasting gold edge'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'nav-auth.js?v=20260522-amber-reuse1' 'nav auth script URL is cache-busted after navbar overflow positioning'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'nav-auth.js?v=20260522-amber-reuse1' 'nav auth include is cache-busted for Amber reuse login fix'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" '20260523-nav-gem-search2' 'auth theme switcher uses current login menu theme cache buster'
...
assert_contains "$blog_get_config_out" '"video_chat":{"participant_limit":8,"token_ttl_seconds":7200,"janus_wss":"wss://janus.example.com/janus","signaling_wss":"wss://signal.example.com/ws","public_rooms":true,"rooms":["Lobby","Office Hours"],"active_rooms":["Lobby","Office Hours"],"scheduled_rooms":"","include_syntax":"{{video-chat}}"}' 'blog-get-config returns video calling settings and include syntax'
