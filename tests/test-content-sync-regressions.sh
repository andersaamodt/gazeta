...
assert_contains "$blog_get_config_out" '"plugins":{"nostr_support":true,"nostr_login":true,"nostr_bridge":true,"nostr_posts":true,"zaps":true,"commerce":true,"btcpay":true,"ramp":false,"merch_store":false,"video_chat":false,"overworld":false}' 'blog-get-config returns normalized plugins json'
