...
EOF_VOTES
  ...
  if blog_list_merge_public_activity_json "$state" "$validation" "alice" | jq -e --argjson expected_next "$expected_next_vote_at" --argjson expected_latest "$latest_vote_at" --argjson current_epoch "$current_epoch" '.elements[0].list_score == 0 and .elements[0].list_latest_vote == -1 and .elements[0].list_latest_vote_created_at == $expected_latest and .elements[0].viewer_vote == -1 and .elements[0].viewer_vote_total == 3 and .elements[0].viewer_can_vote_now == false and .elements[0].viewer_can_change_vote == true and .elements[0].viewer_vote_epoch == $current_epoch and .elements[0].viewer_next_vote_at == $expected_next and .elements[0].vote_cooldown_seconds == 64800' >/dev/null 2>&1; then
    pass
  else
    fail "list vote merge counts every nonzero vote and exposes current viewer metadata"
  fi
}
...
assert_file_contains "$ROOT_DIR/cgi/pre-build" 'page_dest=$(blog_managed_page_path_for_rel "$page_rel")' 'pre-build writes Lodestone projections through managed pages root'
assert_file_not_contains "$ROOT_DIR/cgi/pre-build" 'page_dest="$blog_site_root/site/pages/$page_rel"' 'pre-build does not force Lodestone projections into repo pages'
...
assert_file_contains "$ROOT_DIR/cgi/blog-lib.sh" 'WIZARDRY_PAGES_DIR="$wizardry_pages_dir"' 'async rebuild helper passes explicit generated pages dir'
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" 'prune_clean_url_build_dir=$(blog_generated_build_dir)' 'clean-url pruning uses generated build root'
...
assert_file_contains "$ROOT_DIR/cgi/pre-build" '--html-file "posts_json=$posts_json_tmp"' 'pre-build passes structured public post data into Lodestone'
assert_file_not_contains "$ROOT_DIR/cgi/pre-build" '--set "posts_json=[]"' 'pre-build does not render the blog page with empty post data'
...
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" 'blog_nostr_pages_source_sync_signature "$cfg_json"' 'Nostr source page sync has a deterministic input signature'
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" 'blog_nostr_pages_expected_source_pages_exist "$cfg_json"' 'Nostr source page sync verifies expected canonical sources before skipping'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '--post-tag-year-bg: #dcecff;' 'post year chips use light blue'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/blog-page.js" 'function normalizePostTags(raw)' 'blog list normalizes post tags from catalog data before rendering'
assert_file_contains "$SITE_SOURCE_ROOT/static/blog-page.js" 'return acc.concat(normalizePostTags(post.tags));' 'blog filter options include normalized post tags'
assert_file_contains "$SITE_SOURCE_ROOT/static/blog-page.js" 'var tagsHtml = normalizePostTags(post.tags).map(function (tag) {' 'blog cards render normalized post tags'
...
assert_file_contains "$ROOT_DIR/src/gazeta_read/public_posts.rs" 'read_catalog_with_posts(&static_catalog)' 'fallback blog index rebuilds instead of trusting an empty stale static catalog'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '-webkit-text-fill-color: var(--post-tag-standard-text) !important;' 'single post topic tags force colored text in WebKit'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'staticPrerenderListAdopted: false' 'list hydration tracks adopted static prerendered content'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'function shouldPreserveAdoptedStaticPrerender()' 'list hydration can preserve adopted prerendered content after async refresh'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'shouldRepaint && !shouldPreserveAdoptedStaticPrerender()' 'list hydration skips redrawing adopted static content after async refresh'
...
assert_file_not_contains "$SITE_SOURCE_ROOT/lodestone/pages/blog.stone.html" '<nostr-sync-pill' 'blog Lodestone page does not emit a duplicate static Nostr sync pill'
assert_file_not_contains "$SITE_SOURCE_ROOT/lodestone/templates/list.stone.html" '<nostr-sync-pill' 'list Lodestone template does not emit a duplicate static Nostr sync pill'
assert_file_not_contains "$SITE_SOURCE_ROOT/lodestone/templates/icon-gallery.stone.html" '<nostr-sync-pill' 'icon gallery Lodestone template does not emit a duplicate static Nostr sync pill'
assert_file_not_contains "$SITE_SOURCE_ROOT/lodestone/templates/contact.stone.html" '<nostr-sync-pill' 'contact Lodestone template does not emit a duplicate static Nostr sync pill'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/blog-page.js" 'function pageSyncStatusPillHtml()' 'blog index page renders the shared Nostr sync status pill in the title actions'
assert_file_contains "$SITE_SOURCE_ROOT/static/blog-page.js" 'renderSyncStatusPill();' 'blog index page shows one sync status pill in the canonical title slot'
assert_file_contains "$SITE_SOURCE_ROOT/static/overworld-game.js" 'function renderSyncStatusPill()' 'overworld page renders one sync status pill in the canonical title slot'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'function stripLegacyNostrSyncPills(value)' 'list pages strip legacy static sync pills from editable content'
assert_file_contains "$SITE_SOURCE_ROOT/static/public-ranking-page.js" 'function stripLegacyNostrSyncPills(value)' 'public ranking pages strip legacy static sync pills from editable content'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'data-page="index" data-nav-rank="1">Writing</a>' 'static navbar starts with Writing to match hydrated navbar order'
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'data-page="blog" data-nav-rank="2">Blog</a>' 'static navbar includes Blog after Writing before hydration'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/admin.js" 'data-file-field="mime_type"' 'files admin exposes MIME type editing'
assert_file_contains "$SITE_SOURCE_ROOT/static/admin.js" 'data-file-field="explicit_public"' 'files admin exposes explicit file visibility editing'
...
assert_contains "$admin_preload_body" 'await loadConfig();' 'admin startup loads config for first paint'
assert_contains "$admin_preload_body" 'state.loadedAdminSections.settings = true;' 'admin startup marks settings loaded from config'
assert_not_contains "$admin_preload_body" 'loadNosterRuntime()' 'admin startup does not preload Nostr runtime panel'
assert_not_contains "$admin_preload_body" 'loadZapsRuntime()' 'admin startup does not preload zaps runtime panel'
assert_not_contains "$admin_preload_body" 'loadBtcpayRuntime()' 'admin startup does not preload Lightning runtime panel'
assert_not_contains "$admin_preload_body" 'loadBtcpayCheckoutRuntime()' 'admin startup does not preload BTCPay runtime panel'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/admin.js" 'unsavedPostDraftsByPath[sourcePath] = String((draft && draft.draft_id)' 'admin posts retain edit draft ids for published-post edits'
assert_file_contains "$SITE_SOURCE_ROOT/static/admin.js" "hasUnsavedChanges ? 'Continue editing...' : 'Edit post...'" 'admin post edit action reopens existing edit drafts'
assert_file_contains "$SITE_SOURCE_ROOT/static/admin.js" "Opened the existing unpublished edit for this post." 'admin post edit gives explicit recovery feedback'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'vendor/citrine-nostr-web.js?v=20260622-citrine2' 'Citrine helper is loaded before nav auth'
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'nav-auth.js?v=20260622-citrine2' 'nav-auth include is cache-busted for Citrine adoption'
assert_file_exists "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'tracked Citrine helper is present for Deployments'
...
assert_file_contains "$ROOT_DIR/src/gazeta_read/navbar_pages.rs" 'generated_static_dir().join("navbar-pages.json")' 'navbar endpoint serves prebuilt static navbar payload'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/page-head.js" "var ROUTE_REFRESH_PARAM = '__route_refresh';" 'page head runtime defines stale route refresh sentinel'
assert_file_contains "$SITE_SOURCE_ROOT/static/page-head.js" 'function slugsEquivalent(expected, root)' 'page head runtime normalizes index/blog slug equivalence for route checks'
assert_file_contains "$SITE_SOURCE_ROOT/static/page-head.js" "if (expectedSlug && rootSlug && !slugsEquivalent(expectedSlug, rootSlug))" 'page head runtime detects route/root slug mismatch with slug-equivalence guard'
assert_file_contains "$SITE_SOURCE_ROOT/static/page-head.js" 'showRouteLoadFailure();' 'page head runtime shows explicit route load failure after retry mismatch'
assert_file_contains "$SITE_SOURCE_ROOT/static/page-head.js" 'scheduleRouteLoadFailure();' 'page head runtime defers route failure panel to avoid transient flash during hydration'
assert_file_contains "$SITE_SOURCE_ROOT/static/page-head.js" "window.addEventListener('blog-page-initial-content-ready'" 'page head runtime cancels route failure fallback once initial content paints'
assert_file_contains "$SITE_SOURCE_ROOT/static/page-head.js" 'Page is still loading.' 'page head runtime exposes clear mismatch fallback copy'
...
assert_file_contains "$ROOT_DIR/cgi/blog-lib.sh" 'blog_snapshot_file_before_replace()' 'content writes keep a local snapshot before replacing an existing file'
assert_file_contains "$ROOT_DIR/cgi/blog-lib.sh" 'blog_page_states_dir="$blog_content_root/page-states"' 'editable page state lives under content page-states'
assert_file_contains "$ROOT_DIR/cgi/blog-lib.sh" 'blog_state_markdown_write_json()' 'editable page state writes YAML front matter plus markdown'
assert_file_contains "$ROOT_DIR/cgi/blog-lib.sh" 'blog_state_markdown_to_json()' 'editable page state can be normalized back to JSON for browser and Nostr APIs'
assert_file_contains "$ROOT_DIR/cgi/blog-list-common.sh" 'blog_state_markdown_write_json "$path" "$markdown_state" content' 'list draft saves use YAML front matter plus markdown'
assert_file_contains "$ROOT_DIR/cgi/blog-list-common.sh" 'blog_list_legacy_draft_path "$slug"' 'list draft loading keeps legacy JSON as read-only fallback'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'video-chat-loading-mockup' 'contact page renders a disabled loading fallback when the call widget cannot mount'
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'vcw-shell vcw-shell-no-heading vcw-shell-center-precall' 'contact page fallback uses the final video widget shell classes'
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'vcw-call-owner-btn' 'contact page fallback uses final icon call button controls'
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'markVideoChatUnavailable' 'contact page marks the call widget unavailable when the video script fails'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "'&auto_start=1&mode=' + encodeURIComponent(callMode)" 'Incoming call answer opens the contact call room and auto-starts the requested voice/video mode'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/video-chat-widget.js" "this.startCall(this.state.callMode).catch(function () {" 'video chat widget auto-start query immediately begins the join flow on load'
...
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" 'secure-chat-login-gate' 'contact prerender uses the final Secure Chat logged-out gate'
...
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" 'selected_default_marker_set(entries, show_marker_filters, default_markers)' 'list page prerender hides selected default marker pills like hydration'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'function setInlineDraftField(idx, field, value)' 'list inline editor centralizes field writes through the link-safe setter'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'syncVisibleInlineInputsToDraft();' 'list draft saves flush live inline inputs before persistence'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'externalPostUrlLinkHtml(url, linkTitle)' 'list read mode renders post_url as a separate action when Markdown already has links'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.list-entry-post-url-link' 'list page external post_url action has a compact icon style'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" "document.addEventListener('mousedown', function (event) {" 'list page installs a document-level outside-click handler for row menus'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" "if (state.readRowMenuOpenUid && !target.closest('.list-entry-read-menu')) {" 'read-mode row menu closes when clicking anywhere outside the menu wrapper'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" "'/cgi/blog-delete-list-entry'" 'list page deletes merged public entries through the public-entry store endpoint'
assert_file_contains "$ROOT_DIR/cgi/blog-delete-list-entry" 'map(select((.id // "") != $id))' 'public list entry delete endpoint removes the targeted submission from the public-entry store'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'var(--list-vote-up-stale-color, #6e9586) !important' 'high-specificity stale upvote override stays in the dim green stale color'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.list-entry-vote-controls.is-upvote-only' 'reading-list upvote-only controls have a dedicated layout'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'flex-direction: row;' 'reading-list upvote-only controls keep score to the right of the arrow'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/style.css" '.list-entry-vote-controls.is-upvote-only .list-entry-vote-btn.is-upvote {' 'reading-list upvote-only arrows do not force the active color before or after cooldown'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '--list-vote-up-color: #2f8a5c;' 'default upvotes use a green first-paint color'
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" '--list-vote-up-color: #2f8a5c;' 'Lapidarist upvotes use a green first-paint color'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'function capturePublicSubmitInputForRender()' 'list page public add form snapshots textbox state before hydration repaint'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'restorePublicSubmitInputAfterRender(publicSubmitSnapshot);' 'list page public add form restores focus after hydration repaint'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'input.setSelectionRange(snapshot.selectionStart, snapshot.selectionEnd);' 'list page public add form keeps cursor position after hydration repaint'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '#main-content .list-public-submit .list-public-submit-toggle:hover:not(:disabled)' 'list public add button hover has a high-specificity stable transform rule'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '#main-content .list-public-submit .list-public-submit-toggle-icon' 'list public add button icon has a stable centered rule'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'function markerFilterHashTokensFromLocation()' 'list marker filters can initialize from URL hash tags'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" "token.replace(/_/g, ' ')" 'list marker filter hash tags use underscores for spaces'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" "window.addEventListener('hashchange'" 'list marker filters respond to URL hash changes'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'if (hasMarkerFilterHashOverride()) {' 'list prerender hydration repaints when URL hash tags override defaults'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'border-width: 2px !important;' 'selected marker filter pills get a small size increase only on the selected pill'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'border-color: hsl(var(--marker-pill-h, 220), calc(var(--marker-pill-s, 56%) + 18%), 44%) !important;' 'selected marker filter pills use a bolder same-hue border'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'box-shadow: none !important;' 'selected marker filter pills do not render inner border shadows'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'transform: scale(1.035) !important;' 'selected marker filter pills get selected-only visual size emphasis without row reflow'
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" '#main-content .list-marker-filter-pill.is-include' 'Lapidarist keeps selected marker filter pill borders visible'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'function shouldShowInlineAddEnd()' 'list editor centralizes the bottom add-entry row visibility'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'return !isProductGalleryPage();' 'product gallery edit mode hides the duplicate bottom add-entry row'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'insertAt === 0 && Math.max(0, Number(item && item.depth || 0) || 0) > 0' 'list drag reorder rejects dropping an indented entry into the first list position'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'var marker = normalizeMarkerListText(entry && entry.marker ||' 'marker grouping uses normalized marker labels so equivalent adjacent sections merge'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'function syncVisibleInlineInputsToDraft()' 'list drag reorder syncs visible inline input values before computing grouped sections'
assert_file_contains "$SITE_SOURCE_ROOT/static/list-page.js" 'syncVisibleInlineInputsToDraft();' 'list dragstart flushes edited row fields before reordering'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'body:not(.list-page-wide):not(:has(.post-single-item)) #main-content' 'ordinary public pages keep main content on the shared width lane'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'Shared soft action buttons: borderless tan controls with the Open Chat radius, type, and shadow.' 'title action admin buttons use the shared soft action style'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '--action-soft-shadow:' 'site defines a shared soft action button shadow'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '--action-soft-radius: 8px;' 'site defines the shared Open Chat button radius'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '--action-soft-font-family:' 'site defines shared soft action button typography'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.secure-chat-login-btn {' 'secure chat start/login buttons have explicit soft action styling'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'box-shadow: var(--action-soft-shadow' 'soft action buttons use the Open Chat shadow'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'border-radius: var(--action-soft-radius, 8px)' 'soft action buttons use the Open Chat corner radius'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.nostr-publish-modal-actions .list-admin-primary-btn' 'Nostr publish modal actions use the shared soft action button style'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.blog-compose-btn' 'blog compose action buttons use the shared soft action button style'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.list-page-title-actions button.list-admin-primary-btn' 'title action buttons have a row-specific no-edge reset'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'border-right-width: 0 !important;' 'title action buttons force the right border width to zero'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'border-right-color: transparent !important;' 'title action buttons force the right border color transparent'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'border-image-source: none !important;' 'title action buttons disable border images'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.list-page-title-actions button.list-admin-primary-btn::after' 'title action buttons suppress pseudo-element edge artifacts'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '-webkit-background-clip: border-box;' 'title action parent clears heading gradient clipping away from buttons'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '-webkit-background-clip: text;' 'title action text span keeps the heading gradient'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '0 0 0 1px var(--border),' 'navbar overflow badge paints its ring outside the badge fill'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '0 0 0 3px var(--button-secondary-bg, var(--nav-button-bg, var(--light-bg)));' 'navbar overflow badge masks the icon underneath with an opaque halo'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.nav-overflow-menu:hover .nav-menu-btn.nav-overflow-btn' 'navbar overflow wrapper hovers the icon and badge together'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '0 0 0 3px rgba(15, 23, 42, 0.10);' 'navbar overflow badge halo matches the hover surface'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" 'Shared soft action buttons: borderless tan controls with the Open Chat radius, type, and shadow.' 'Lapidarist title action admin buttons use the shared soft action style'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" 'border-right-width: 0 !important;' 'Lapidarist title action buttons force the right border width to zero'
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" '.list-page-title-actions button.list-admin-primary-btn::after' 'Lapidarist title action buttons suppress pseudo-element edge artifacts'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/head.html" '20260622-lapidarist-restore1' 'head theme stylesheet cache buster tracks parchment texture fixes'
assert_file_contains "$SITE_SOURCE_ROOT/includes/head.html" '__wizardryThemeStylesheetVersion' 'head exports the shared theme stylesheet version for hydrated page code'
assert_file_contains "$SITE_SOURCE_ROOT/static/admin.js" '20260622-lapidarist-restore1' 'admin theme preview cache buster tracks parchment texture fixes'
assert_file_contains "$SITE_SOURCE_ROOT/static/page-head.js" '__wizardryThemeStylesheetVersion' 'page head reconciliation reuses the shared theme stylesheet version'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/head.html" 'background: var(--post-tag-year-bg) !important;' 'shared head recovery forces year pills light blue'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.site-footer {' 'footer shell has dedicated layout styling'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.footer-version {' 'footer version has dedicated styling'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'function signInHelperMessage' 'Citrine owns one generic account-creation helper across methods'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'First successful sign-in creates your account automatically.' 'Sign-in helper does not make account creation sound browser-only'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'window.citrine.web.signInHelperMessage(tab)' 'Gazeta consumes Citrine sign-in helper copy'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-platform-grid' 'Sign-in modal uses guided platform choices instead of tabs'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "authPlatformButton('auth-tab-register', 'register', 'desktop', true, 'desktop', 'Desktop')" 'Sign-in modal desktop platform button has no subtitle'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "authPlatformButton('auth-tab-phone', 'phone', 'android', false, 'android', 'Android')" 'Sign-in modal Android platform button has no subtitle'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "authPlatformButton('', 'phone', 'ios', false, 'ios', 'iPhone')" 'Sign-in modal iOS platform button has no subtitle'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "authPlatformButton('', 'phone', 'remote', false, 'remote', 'Remote Signer')" 'Sign-in modal remote signer platform button has no subtitle'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'class="auth-advanced-toggle"' 'Manual sign-in is hidden behind an advanced link'
...
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Firefox signer' 'Sign-in modal removes desktop platform subtitle'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Amber signer' 'Sign-in modal removes Android platform subtitle'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Safari signer' 'Sign-in modal removes iOS platform subtitle'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Nostr Connect</span>' 'Sign-in modal removes remote signer platform subtitle'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" '<span class="auth-platform-copy"><strong>Manual</strong></span>' 'Manual sign-in is not shown as a platform card'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Recommended Apps' 'Sign-in modal combines login and zap recommendations into one app island'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-apps-icon' 'Recommended Apps header uses an app-grid icon'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-login-icon' 'Recommended Apps header no longer uses the lock/login icon'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-login-apps' 'Sign-in modal keeps login recommendation app list'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-zap-apps' 'Sign-in modal keeps zap recommendation app list'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'For Login:' 'Sign-in modal does not split recommendations into a login island'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'For Zaps:' 'Sign-in modal does not split recommendations into a zap island'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'class="auth-tab-frame"' 'Sign-in modal keeps the login flow before app recommendations'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-login-onboarding' 'Sign-in modal renders the login recommendation group'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-zap-onboarding' 'Sign-in modal renders the zap recommendation group'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Recommendations Updated May 2026' 'Recommended app island shows freshness date'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Self-Sovereign' 'Zap onboarding no longer shows a mode selector'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Quick &amp; Easy' 'Zap onboarding no longer recommends quick custodial options'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'function nostrLoginRecommendation' 'Citrine login recommendations update by selected platform'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'function recommendationPlatformLabel' 'Recommendation groups derive a visible platform label from the selected login platform'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "platformLabel + ' login'" 'Login recommendation group includes the selected platform'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Recommended for Android Nostr Connect login.' 'Android login recommendation explains that it is platform-specific'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Recommended for iPhone and iPad Nostr Connect login.' 'iOS login recommendation explains that it is platform-specific'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Recommended for remote Nostr Connect signers.' 'Remote signer login recommendation explains that it is platform-specific'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "purpose: 'Login via Nostr'" 'Login recommendation apps are individually labeled as login apps'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "platform.className = 'auth-reco-platform'" 'Recommendation app rows render a visible platform badge'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Amber' 'Android login onboarding recommends Amber'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'function nostrZapRecommendation' 'Citrine zap onboarding recommendations update by selected platform'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "platformLabel + ' zaps'" 'Zap recommendation group includes the selected platform'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Recommended for sending zaps from Android.' 'Android zap recommendation explains that it is platform-specific'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Recommended for sending zaps from iPhone and iPad.' 'iOS zap recommendation explains that it is platform-specific'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Recommended when the signer is remote or the current platform is unknown.' 'Remote zap recommendation explains that it is platform-specific'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "purpose: 'Zaps: Lightning wallet'" 'Zap recommendation apps identify Lightning wallet purpose'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "purpose: 'Zaps: Nostr client'" 'Zap recommendation apps identify Nostr client purpose'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Amethyst' 'Android zap onboarding recommends Amethyst'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'ZEUS' 'Zap onboarding recommends ZEUS for self-custodial Lightning'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "source: 'F-Droid', label: 'Download Amber', url: 'https://f-droid.org/packages/com.greenart7c3.nostrsigner/'" 'Android login onboarding links directly to Amber on F-Droid'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Choose a sign-in method. Your Nostr public key is your account, and the site never asks for a private key.' 'Sign-in modal helper combines opening guidance into the top toast'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Choose where your signer lives.' 'Sign-in modal avoids vague signer-location wording'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "return base + ' Connect Nostr with the link or QR. Sign-in continues after pairing.'" 'Android phone signer guidance is combined into the generic helper toast'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-phone-intro' 'Phone signer guidance no longer interrupts the platform buttons and QR'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "label: 'Download ZEUS'" 'Zap onboarding uses explicit Download app-name link text'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "stores.appendChild(doc.createTextNode(source + ': '));" 'Recommendation download links show their F-Droid or Aurora source separately from the Download link'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'https://github.com/vitorpamplona/amethyst#installation' 'Amethyst app link points to its official installation page'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'https://github.com/ZeusLN/zeus#app-store-links' 'ZEUS app link points to its official app store links'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "label: 'Play'" 'Recommendation lists never recommend Play Store'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'play.google.com' 'Recommendation lists never link to Play Store'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'https://apps.apple.com/us/app/damus/id1628663131' 'Damus App Store link points to its app listing'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'https://nostur.com/appstore' 'Nostur App Store link points to its app listing'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-reco-app-link' 'Login and zap onboarding render compact linked app chips'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-reco-app-purpose' 'Login and zap onboarding render explicit app purpose labels'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'https://auroraoss.com/downloads/AuroraStore/' 'Zap onboarding links Aurora Store'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Wallet of Satoshi' 'Zap onboarding does not recommend custodial wallets'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Primal' 'Zap onboarding does not recommend quick hosted Nostr apps'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Nsec.app' 'Zap onboarding does not recommend quick remote signer services'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'not maximum sovereignty' 'Zap onboarding no longer labels custodial tradeoffs because those options are removed'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Back up the seed' 'Zap onboarding avoids long wallet setup notes'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'function authRecommendationIconAssets' 'Gazeta passes cached local app assets into Citrine recommendations'
...
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-phone-reco' 'phone signer recommendations are not repeated below the panel actions'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Continue with browser signer' 'Browser signer action avoids special register wording'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'You can change your username after you log in.' 'Sign-in modal defers username choice until after login'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-register-username' 'Sign-in modal does not ask for a username during first login'
...
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Usernames must be unique.' 'Sign-in modal avoids username uniqueness friction during login'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" '>Advanced...</button>' 'Manual challenge fallback is reachable through the advanced link'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Challenge JSON</span>' 'Sign-in modal removes manual platform subtitle'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Register with browser signer' 'Browser signer path no longer uses special register wording'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Signed challenge</button>' 'Manual fallback avoids cryptic signed challenge tab wording'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Your Nostr public key is your account' 'Sign-in modal includes concise Nostr identity guidance in the top helper'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Browser sign-in uses a NIP-07 extension' 'Sign-in modal explains browser signer path'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Connect Nostr with the link or QR. Sign-in continues after pairing.' 'Sign-in modal explains phone signer path without interrupting the QR'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'nostr-info-modal' 'Learn about Nostr sign-in no longer uses a separate modal'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'nav-auth.js?v=20260622-citrine2' 'nav auth script URL is cache-busted after Citrine adoption'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'window.citrine.nostr.buildNostrConnectUri' 'Nostr Connect link generation uses Citrine'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "perms: 'get_public_key,sign_event:22242,sign_event:9734,sign_event:7,sign_event:17,sign_event:5'" 'Nostr Connect link requests login, zap, vote, and cleanup signing permissions through Citrine'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'metadata: metadata' 'Nostr Connect link includes app metadata for signer pairing through Citrine'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'typeof msg.result ===' 'Citrine accepts connect responses that return the secret as result'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'function getNip46AccountPubkeyWithRetry' 'phone signer login retries account pubkey requests after signer connect ack'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'var delays = [1200, 2400, 4800];' 'phone signer account pubkey retry gives mobile signer relay listeners a settle window'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'window.citrine.nostr.withTimedOutRetry' 'phone signer account pubkey retry uses Citrine when available'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "getNip46AccountPubkeyWithRetry({ forceRefresh: true })" 'phone signer login refreshes the account pubkey for each login attempt'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "window.addEventListener('pageshow'" 'phone signer refreshes when the browser restores the page after signer return'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'function refreshUnpairedNip46Link' 'Connect Nostr refreshes unpaired NIP-46 state before opening the signer'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'resetPhonePairingLink({ rotateAppKey: true })' 'unpaired Connect Nostr clicks use a new client key instead of replacing signer connections'
...
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-phone-connect-btn' 'phone signer panel does not show redundant QR connect button'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" "signer.method === 'nip46'" 'shared signer status treats restored phone signer pairing as available without forcing a fresh QR'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-nip46-diagnostics' 'phone signer panel shows visible Nostr Connect pairing diagnostics'
...
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-nip46-amber-open' 'phone signer panel avoids app-specific fallback links'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'id="auth-nip46-copy"' 'phone signer panel avoids redundant copy control above URI box'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-nip46-pairing' 'phone signer QR and controls share one side-by-side pairing layout'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Phone signer login steps' 'phone signer panel shows explicit Android-friendly steps'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'Return here after approval.' 'phone signer steps explain the Android app-switch return'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-nip46-open" class="auth-secondary-btn auth-nip46-open-link"' 'phone signer panel shows a button-styled Nostr Connect opener'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" '>Copy link<' 'phone signer copy fallback avoids text button chrome'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-nip46-uri-copy' 'phone signer URI box includes one-click copy button'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'id="auth-nip46-uri" hidden' 'phone signer hides the raw Nostr Connect URI by default'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" '>Connect Nostr<' 'phone signer primary link uses clearer wording'
assert_file_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'hidden>Continue sign-in</button>' 'phone signer continue button starts hidden until a signer is paired'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'els.authNip46Qr.hidden = paired' 'paired phone signer state hides the QR that would replace the signer connection'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "setNip46Diagnostics('Signer connected. Continue sign-in.', 'ok')" 'paired phone signer state explains that login can continue without reconnecting the signer'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'Phone signer is already paired. Continue sign-in and approve the login request in your signer.' 'paired phone signer state gives signer-neutral continuation guidance'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'approve the login request in Amber' 'paired phone signer continuation guidance is not Amber-specific'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "openNativeDeepLink(currentNip46Uri(), 'Phone signer link is not ready yet.')" 'paired phone signer login reopens the saved signer connection without rotating it'
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'nav-auth.js?v=20260622-citrine2' 'nav auth include is cache-busted for Citrine adoption'
...
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'auth-nip46-reset' 'phone signer panel omits fresh-link recovery from primary onboarding'
...
assert_file_not_contains "$SITE_SOURCE_ROOT/static/vendor/citrine-nostr-web.js" 'id="auth-nip46-open" class="auth-inline-link" href="#" target="_blank"' 'Nostr Connect link avoids target blank so Android can open signer apps reliably'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'id="nav-menu-logout-more"' 'user menu hides secondary logout actions behind a compact overflow trigger'
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'id="nav-menu-logout-submenu"' 'user menu renders secondary logout actions in a logout submenu'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'menuLogoutSubmenu' 'nav auth controls the logout submenu state'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.nav-menu-logout-row' 'logout menu item reserves a right-side overflow button'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" '#nip23-page-title .list-page-title-actions .list-admin-primary-btn' 'lapidarist nip23 title buttons use the shared soft action contract'
assert_file_contains "$SITE_SOURCE_ROOT/static/themes/lapidarist.css" '#public-ranking-title .list-page-title-actions .list-admin-primary-btn' 'lapidarist public ranking title buttons use the shared soft action contract'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" '20260622-lapidarist-restore1' 'auth theme switcher uses current parchment texture theme cache buster'
...
assert_file_contains "$ROOT_DIR/cgi/blog-open-post" 'blog_generated_post_html_path_for_file "$md_file"' 'single post route resolves prerendered html through the shared generated-build helper'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/post-context.js" 'function postSyncStatusPillHtml(payload)' 'single post client renders the shared Nostr sync status pill'
assert_file_contains "$SITE_SOURCE_ROOT/static/post-context.js" 'renderPostSyncStatusPill(payload, layout);' 'single post enhancement mounts the sync pill during hydration'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.post-sync-status-row {' 'single post header reserves space for the sync status pill'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/head.html" '20260615-post-sync-pill1' 'shared post context script cache buster tracks the sync pill update'
assert_file_not_contains "$SITE_SOURCE_ROOT/includes/head.html" '/static/post-resize.js' 'blog pages no longer load the hidden persistent post resize behavior'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/style.css" 'body.blog-post-resize-enabled' 'public layout no longer widens around a hidden resize mode'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/style.css" '.post-item.blog-post-resizable' 'public post cards no longer carry resize-specific classes'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'width: 100%;' 'single post island uses the normal blog page width'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'body:has(.post-single-item) {' 'single post body route gets its own desktop width cap'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'max-width: 800px;' 'single post body cap matches the blog index page width'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'max-width: 72rem;' 'wide single post body cap matches the wide post island width'
assert_file_contains "$ROOT_DIR/cgi/blog-open-post" "style_css_version='20260622-lapidarist-restore1'" 'single post fallback route cache-busts the parchment and tag styles'
assert_file_contains "$ROOT_DIR/cgi/blog-open-post" 'cat "$head_file"' 'single post fallback route includes the normal theme stylesheet head'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/style.css" '.post-single-item.blog-post-resizable' 'single post layout no longer depends on resizable card modifiers'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'width: min(100%, 800px) !important;' 'single post navbar follows the normal blog page width'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" 'padding: 1.75rem 3.25rem 1.55rem;' 'single post island keeps the restored desktop reading padding'
...
assert_file_not_contains "$SITE_SOURCE_ROOT/static/style.css" 'blog-post-resize-enabled' 'mobile layout no longer carries hidden resize-mode containment branches'
...
assert_contains "$blog_get_config_out" '"plugins":{"nostr_support":true,"nostr_login":true,"nostr_bridge":true,"nostr_posts":true,"zaps":true,"commerce":true,"btcpay":true,"crossmint":false,"merch_store":false,"video_chat":false,"overworld":false}' 'blog-get-config returns normalized plugins json'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nip23-page.js" 'function productPriceDisplayText' 'nip23 product pages render a standalone price display from per-page price'
assert_file_contains "$SITE_SOURCE_ROOT/static/nip23-page.js" "return 'Free';" 'nip23 product pages render 0.00 as Free'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.nip23-product-page-price {' 'nip23 product pages style the product price display'
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.nip23-product-landing {' 'nip23 product pages have reusable app landing-page layout styles'
...
assert_file_contains "$ROOT_DIR/cgi/blog-payments-legacy" 'crossmint_client_api_key:' 'payments status emits Crossmint runtime key for checkout embeds'
...
assert_file_contains "$ROOT_DIR/cgi/blog-manage-merch" 'sync_selected' 'merch manager can sync selected Printful products'
assert_file_contains "$ROOT_DIR/cgi/blog-manage-merch" 'rebuild_merch_landing_page' 'merch manager rebuilds the unlisted merch landing page'
...
assert_file_contains "$ROOT_DIR/cgi/blog-lib.sh" 'crossmint' 'plugin registry includes Crossmint plugin'
assert_file_contains "$ROOT_DIR/cgi/blog-lib.sh" 'commerce' 'plugin registry includes shopping cart plugin'
assert_file_contains "$SITE_SOURCE_ROOT/pages/admin.md" 'data-plugin-row="commerce"' 'admin plugin table exposes shopping cart plugin'
...
assert_file_contains "$SITE_SOURCE_ROOT/pages/admin.md" 'data-plugin-row="crossmint"' 'admin plugin table exposes Crossmint plugin'
assert_file_contains "$SITE_SOURCE_ROOT/pages/admin.md" 'data-admin-section="merch"' 'admin includes merch curation section scaffold'
assert_file_contains "$SITE_SOURCE_ROOT/pages/admin.md" 'id="merch-products-list"' 'admin merch section includes product list container'
assert_file_contains "$SITE_SOURCE_ROOT/static/admin.js" '/cgi/blog-manage-merch' 'admin merch curation talks to merch manager endpoint'
assert_file_contains "$SITE_SOURCE_ROOT/static/admin.js" 'data-merch-show-id' 'admin merch curation renders show-on-merch checkboxes'
assert_file_contains "$SITE_SOURCE_ROOT/static/admin.js" "action: 'sync_selected'" 'admin merch curation can sync selected products'
...
assert_file_contains "$ROOT_DIR/cgi/blog-delivery" "style_css_version='20260622-lapidarist-restore1'" 'delivery page cache-busts the restored parchment and tag styles'
