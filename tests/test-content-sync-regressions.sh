...
assert_file_contains "$SITE_SOURCE_ROOT/lodestone/pages/blog.stone.html" '<nostr-sync-pill slug={slug}></nostr-sync-pill>' 'blog Lodestone page includes a prerendered Nostr sync pill'
assert_file_contains "$SITE_SOURCE_ROOT/lodestone/templates/list.stone.html" '<nostr-sync-pill slug={slug}></nostr-sync-pill>' 'list Lodestone template includes a prerendered Nostr sync pill'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/footer.md" 'v0.7.26' 'footer exposes current site version before GitHub'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'window.CitrineNostrWeb.signInHelperMessage(tab)' 'Gazeta consumes Citrine sign-in helper copy'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'window.CitrineNostrWeb.buildNostrConnectUri' 'Nostr Connect link generation uses Citrine'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'window.CitrineNostrWeb.withTimedOutRetry' 'phone signer account pubkey retry uses Citrine when available'
