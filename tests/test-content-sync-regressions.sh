...
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" "blog_page_js_version='20260611-blog-defaulttag-prerender1'" 'blog page script cache buster tracks default-tag prerender updates'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/blog-page.js" 'pageSyncStatusPillHtml()' 'blog page can render the Nostr sync status pill in the title actions'
assert_file_contains "$SITE_SOURCE_ROOT/static/blog-page.js" 'renderSyncStatusPill();' 'blog page shows the sync status pill for non-admin visitors'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" '20260526-title-action-edge1' 'auth theme switcher uses current title action edge theme cache buster'
