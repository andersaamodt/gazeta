...
assert_file_not_contains "$ROOT_DIR/site/static/overworld-game.js" "startDownload({ fromCache: true })" 'web game does not autostart when the current build is cached'
assert_file_contains "$ROOT_DIR/site/static/overworld-game.js" "status.textContent = 'Cached Godot Overworld is ready'" 'web game reports cached readiness without booting Godot'
