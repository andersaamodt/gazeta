...
CSS="$ROOT_DIR/site/static/artificer-web-page.css"
NAV="$ROOT_DIR/site/static/nav-auth.js"

assert_file_contains "$PAGE" 'title: Artificer Web' "artificer page has title"
assert_file_contains "$PAGE" '/static/artificer-web-page.css?v=20260529-artificer-web9' "artificer page loads cache-busted shell styles"
assert_file_contains "$PAGE" '/static/artificer-web-page.js?v=20260529-artificer-web9' "artificer page loads cache-busted shell script"
...
assert_file_contains "$JS" "window.blogAuth.startLogin({ fallbackModal: false })" "shell starts the primary Nostr signer login flow without opening helper UI"
assert_file_contains "$JS" "isAuthFailure" "shell treats bridge auth errors as login gate states"
...
assert_file_contains "$CSS" 'body.auth-modal-open .artificer-web-shell' "artificer shell drops behind the auth modal"
assert_file_not_contains "$CSS" 'border-left: 3px solid' "artificer auth/error shell avoids left-border callout styling"
