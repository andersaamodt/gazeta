...
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-map-create-btn,' 'transition: none !important;' 'Desk map button hover states are instant'
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-map-create-btn *,' 'transition: none !important;' 'Desk map button icon hover states are instant'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'grouped[segment.key].push(segment);' 'Desk wall lamps merge exposed wall segments into evenly spaced runs'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'var count = Math.max(1, Math.round(length / 96));' 'Desk wall lamps are evenly spaced along room and hallway wall runs'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'desk-map-greenbelt-strip desk-map-greenbelt-fade far' 'Desk greenbelt renders the far outer fade on the same plain perimeter'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'desk-map-greenbelt-strip desk-map-greenbelt-fade mid' 'Desk greenbelt renders the middle outer fade on the same plain perimeter'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'desk-map-greenbelt-strip desk-map-greenbelt-fade near' 'Desk greenbelt renders the near outer fade on the same plain perimeter'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'stroke-width: 92;' 'Desk greenbelt reserves only the outer edge for a gentle fade'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'stroke-width: 84;' 'Desk greenbelt uses a middle fade stroke for a smoother edge'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'stroke-width: 76;' 'Desk greenbelt uses a near fade stroke for a seamless edge'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'opacity: 0.025;' 'Desk greenbelt far edge fade is subtle'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'opacity: 0.12;' 'Desk greenbelt near edge fade blends into the opaque band'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "mode === 'map' || ((mode === 'todo' || mode === 'compose') && state.paperMapVisible)" 'Desk only treats paper modes as map-visible when the side map is actually mounted'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "(renderPaperMap ? renderMapSafely(data) : '') + (isPaperSwitching && state.paperSwitchFrom === 'todo' ? renderTodo(data) : '') + renderCompose(data)" 'Desk compose mode only mounts the map when compose was opened from an already-visible map'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "(renderPaperMap ? renderMapSafely(data) : '') + (isPaperSwitching && state.paperSwitchFrom === 'compose' ? renderCompose(data) : '') + renderTodo(data)" 'Desk checklist stays usable without forcing the map open and can overlap outgoing compose'
