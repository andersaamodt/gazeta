...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "state.mapZoomMode = state.mapZoomMode === 'room' ? 'full' : 'room';" 'Desk map zoom toggle switches between closeup and whole-map modes'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'background: transparent !important;' 'Desk map utility icons have no square idle background'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'background-image: none !important;' 'Desk map utility icons suppress inherited button chrome while idle'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-room-name-title::after' 'Desk todo room heading masks the full-width notebook rule on the title row'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function switchPaperModeWithClose(previousMode, nextMode)' 'Desk switches between notepad and composition book through the close animation'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'state.mode = nextMode;' 'Desk opens the requested paper panel only after the old one closes'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'desk-map-room-grass-edge-soft' 'Desk outdoor areas render one soft green edge gradient'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-map-room-grass-edge-core' 'Desk outdoor areas share the greenbelt core fringe width'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'desk-map-greenbelt-gradient' 'Desk greenbelt itself uses a gradient stroke'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function outdoorFringePath' 'Desk outdoor fringe is computed from exposed outdoor edges'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "if (local[x + ',' + y]) return false;" 'Desk outdoor fringe skips internal outdoor room edges'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'if (owner == null) return true;' 'Desk outdoor fringe avoids double-fading against building walls'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'background: #e8cf8b !important;' 'Desk map corner buttons use a solid light map-color hover background'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'opacity: calc((1 - var(--presence)) * 0.48);' 'Desk map makes fully dim indoor rooms visibly darker'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'desk-map-presence-glow' 'Desk presence uses a radiant gradient aura'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" '<clipPath id="' 'Desk presence aura is clipped to room walls'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'clip-path="url(#' 'Desk presence aura fills the room through a clipped rectangle'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'var overlayPad = Math.max(unitW, unitH);' 'Desk presence and dim overlays extend across decorative wall contours'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'renderGreenbelt(buildingCells, buildingOccupied' 'Desk greenbelt follows the contoured building footprint, not outdoor room walls'
...
assert_file_not_contains "$ROOT_DIR/site/static/desk-page.js" 'desk-map-greenbelt-soft' 'Desk greenbelt does not add a separate gradient overlay path'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'stroke-width: 44;' 'Desk greenbelt uses one broad perimeter planting stroke'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'stroke: url(#desk-map-greenbelt-gradient);' 'Desk greenbelt strip carries its own gradient'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" '<stop offset="0.28" stop-color="#87936a"' 'Desk greenbelt gradient keeps the muted rustic map green in the belt'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "mode === 'map' || mode === 'todo' || mode === 'compose'" 'Desk treats checklist and compose modes as map-visible modes'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "state.mode = 'map';" 'Desk checklist close returns to map mode instead of closing the whole stage'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "content = renderMapSafely(data) + renderCompose(data);" 'Desk compose mode keeps the map open and replaces the checklist panel'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'data-desk-doc-drag' 'Desk compose exposes a drag handle for moving open documents between rooms'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-compose-drag-handle:hover' 'Desk compose drag handle appears only when directly hovered'
assert_file_not_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-compose-panel:hover .desk-compose-drag-handle-wrap' 'Desk compose drag handle no longer appears from hovering the whole paper'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'place-items: center;' 'Desk compose drag handle centers its glyph'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'border-radius: 4px;' 'Desk compose drag handle uses square-corner geometry'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'box-shadow: none;' 'Desk compose drag handle avoids a floating drop shadow'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "api('move-document'" 'Desk compose drag drop moves documents between rooms'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "event.target.closest('.desk-map-room-title') && clickedRoom === state.currentRoom" 'Desk map ignores single clicks on the current room title before a double-click rename'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "event.target.closest('.desk-map-room-title') && (event.shiftKey || roomPath === state.currentRoom)" 'Desk map double-click edits the current room title instead of opening the notepad'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'renderMapSafely(data) + renderTodo(data)' 'Desk checklist stays usable even if the map render fails'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '--desk-notebook-right-margin: 0.9rem;' 'Desk checklist keeps a comfortable right page margin'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'max-width: calc(100% - var(--desk-notebook-margin) - var(--desk-notebook-writing-inset) - var(--desk-notebook-right-margin));' 'Desk checklist add line respects the wider right page margin'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'align-self: end;' 'Desk checklist checkbox sits on the notebook line baseline'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'forgotten_tasks' 'Desk frontend renders a separate forgotten task drawer'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'data-desk-forgotten-toggle' 'Desk forgotten task section expands from the notebook'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-forgotten-drawer' 'Desk forgotten drawer animates open on notebook lines'
