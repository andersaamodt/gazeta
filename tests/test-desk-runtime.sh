...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function roomWallLightPoints' 'Desk map places wall lights along the longer exposed room walls'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'desk-map-door-spill' 'Desk map lets brighter rooms spill light through doorway CSS'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-map-door-spill' 'Desk map styles doorway light spill without changing the light model'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "renderMapSafely(data) + (isPaperSwitching && state.paperSwitchFrom === 'todo' ? renderTodo(data) : '') + renderCompose(data)" 'Desk compose mode keeps the map open and can overlap the outgoing checklist panel'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "renderMapSafely(data) + (isPaperSwitching && state.paperSwitchFrom === 'compose' ? renderCompose(data) : '') + renderTodo(data)" 'Desk checklist stays usable even if the map render fails and can overlap outgoing compose'
