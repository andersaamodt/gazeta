...
assert_file_contains "$ROOT_DIR/site/pages/desk.md" 'desk-texture-clean1' 'Desk page cache-busts cleaned room and grass textures'
assert_file_exists "$ROOT_DIR/site/static/fonts/patrick-hand-latin-400-normal.woff2" 'Desk bundles a free-use handwriting font for the todo pad'
assert_file_contains "$ROOT_DIR/site/static/fonts/OFL-patrick-hand.txt" 'SIL OPEN FONT LICENSE' 'Desk includes the bundled handwriting font license'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'grid-template-columns: 1fr;' 'Desk checklist does not reflow the map when opened'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'right: clamp(1rem, 3vw, 2.2rem);' 'Desk checklist opens as a floating vertical sheet beside the stable map'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'font-family: "Desk Patrick Hand"' 'Desk todo pad uses a print handwriting font'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'height: clamp(20rem, calc((var(--todo-lines) + 3) * var(--desk-notebook-line)), min(46.7rem, 88%));' 'Desk todo pad expands from a wide starter ratio toward A4 proportions'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'stroke="rgba(112,57,31,0.22)"' 'Desk room parchment pattern includes visible room-local grid lines'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'rgba(112,57,31,0.22)' 'Desk map room parchment texture has visible warmer grid lines'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'background: linear-gradient(180deg, var(--desk-gold-soft), var(--desk-gold));' 'Desk secret passage trigger matches the create-room gold circle'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function statusTooltip' 'Desk status icons explain what each icon does'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'wrap="off"' 'Desk checklist add miniform keeps new task text on one line'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'data-desk-todo-add-toggle' 'Desk checklist add-task control expands from a plus button'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-todo-add-reveal' 'Desk checklist add-task fields reveal like the Reading List add control'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'grid-template-columns: 0fr;' 'Desk checklist add-task form starts collapsed'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-todo-add.is-expanded .desk-todo-add-toggle-icon' 'Desk checklist add-task plus rotates into a close control'
