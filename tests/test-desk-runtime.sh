...
assert_file_contains "$ROOT_DIR/site/pages/desk.md" '20260530-todo-row-props-grass1' 'Desk page cache-busts latest Desk todo row, properties, and grass polish'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-mode-dock {' 'Desk lower mode launchers share the desktop width instead of drifting with wider windows'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'left: clamp(0.35rem, 0.8vw, 0.75rem);' 'Desk map launcher stays diagonally under the desktop corner'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'right: clamp(0.35rem, 0.8vw, 0.75rem);' 'Desk todo launcher stays diagonally under the opposite desktop corner'
...
assert_file_not_contains "$ROOT_DIR/site/static/desk-page.js" 'data-desk-map-greenbelt-tone' 'Desk map no longer exposes the temporary greenbelt color test toggle'
...
assert_file_not_contains "$ROOT_DIR/site/static/desk-page.js" 'function renderGreenbeltToneIcon' 'Desk map no longer carries temporary greenbelt toggle icon code'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function attachmentVectorAwayFromParent' 'Desk subdivision layout knows which way connected room containers should expand'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'containedRectExpansionPenalty' 'Desk subdivision layout prefers expanding away from the attached parent room'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'collisions * 100000' 'Desk subdivision layout strongly avoids overlapping occupied outdoor cells'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function preserveOpenPaperMode()' 'Desk room navigation snapshots an open notepad or compose page before loading the next room'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function restoreOpenPaperMode(snapshot)' 'Desk room navigation restores the open paper mode after the next room loads'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'setRoom(clickedRoom, false, { preserveOpenPaper: true, previewedMapNavigation: true })' 'Desk map room clicks keep an open notepad or compose page visible without replaying the zoom'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'setRoom(roomPath, false, { preserveOpenPaper: true })' 'Desk double-click room travel keeps an open notepad or compose page visible'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "clickedTitle.getAttribute('data-desk-room-title') || '') === state.currentRoom" 'Desk map room labels only intercept clicks for the current room rename affordance'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-map-room-link:not(.is-current) .desk-map-room-title' 'Desk map labels for other rooms show a navigation pointer'
...
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '#desk-page-root button,' 'box-shadow: none !important;' 'Desk all in-app buttons have no drop shadows'
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '#desk-page-root button,' '-webkit-box-shadow: none !important;' 'Desk all in-app buttons suppress Safari button shadows'
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" 'body.desk-page-body nav.site-nav button,' 'box-shadow: none !important;' 'Desk top-right nav buttons have no drop shadows'
...
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-room-name-title::before {' 'content: none;' 'Desk todo room heading suppresses the extra long notebook-rule underline'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'desk-set-square-body' 'Desk map properties button uses a ruled set-square icon'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-set-square-rules' 'Desk map properties set-square icon draws ruler marks'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function settingsGearIcon()' 'Desk settings button renders the shared Wizardry stock gear icon inline'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'M14 8.3V5.73' 'Desk settings button uses the stock Wizardry gear path'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function userMenuDotsIcon()' 'Desk user menu button is normalized to a three-dot icon'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-user-menu-dots-icon' 'Desk user menu three-dot icon is explicitly sized and filled'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-settings-menu-btn > :not(.desk-settings-gear-icon)' 'Desk settings button hides stale menu glyphs'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-user-menu-btn > :not(.desk-user-menu-dots-icon)' 'Desk user menu button hides stale extra glyphs'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function containedTreePaths(parentPath)' 'Desk contained-room layout tracks a nested subdivision ownership tree'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'collisions * 10000' 'Desk contained-room layout strongly avoids expanding over unrelated rooms'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function rectCells(rect)' 'Desk contained-room occupancy expands rectangular shares into grid cells'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'layout[parentPath].selfBox' 'Desk subdivided parents remember the actual parent share for connected child placement'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function parentAttachmentCells(parentPath)' 'Desk connected children attach to the parent share instead of the whole subdivided container'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function compactDirectionNamesForParent(parentPath)' 'Desk connected children of subdivided parents prefer direct edge attachments'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "['north', 'east', 'west', 'south']" 'Desk connected children of subdivided parents prefer a north-edge attachment when it is open'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'var directionPreferencePenalty = layout[parentPath] && layout[parentPath].containedSelf ? orderIndex * 240 : orderIndex * 0.7;' 'Desk connected child placement does not let map balance override the subdivided parent edge order'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'gradientEdges = mergedEdges.map(gradientEdge).join' 'Desk greenbelt builds the outer fade from merged exterior edge geometry'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'var gradientWidth = 84;' 'Desk greenbelt outer edge fade is wide and gradual'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'var gradientOverlap = 2;' 'Desk greenbelt fade overlaps the opaque band without creating corner spikes'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'var gradientCapOverlap = 0;' 'Desk greenbelt fade ends at corner gradients instead of extending past them'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function mergeGreenbeltEdges' 'Desk greenbelt merges straight perimeter edges before drawing the fade'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'var greenbeltPaths = mergedEdges.map(function (edge)' 'Desk greenbelt uses a clean merged perimeter instead of cell-sized fade rectangles'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'stroke-width: 60;' 'Desk greenbelt keeps a clean but thinner opaque main band'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'stop-opacity="0.13"' 'Desk greenbelt outer edge uses a gentler multi-stop fade'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'stroke: var(--desk-greenbelt-color, #9fac63);' 'Desk greenbelt uses the same selected green as outdoor rooms'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'fill: var(--desk-greenbelt-color, #9fac63);' 'Desk outdoor areas use the same selected green as the greenbelt'
assert_file_not_contains "$ROOT_DIR/site/static/desk-page.js" 'desk_greenbelt_tone_v1' 'Desk no longer stores a temporary greenbelt color toggle'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "return '#8d9a54';" 'Desk uses the preferred slightly lightened darker grass color'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'renderGreenbelt(greenbeltCells, greenbeltOccupied, unitW, unitH, mapGreen)' 'Desk greenbelt gradient receives the selected green'
...
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-map-room-dim.is-outdoor {' 'opacity: 0;' 'Desk outdoor rooms are not darkened against the greenbelt'
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-map-room-presence.is-outdoor {' 'opacity: 0;' 'Desk outdoor rooms do not glow away from the greenbelt color'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function secretDoorCandidates' 'Desk secret passages choose door points from room wall edges'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function chooseSecretDoorPoint' 'Desk secret passages choose real wall door slots instead of projecting over contours'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'usedDoorSlots[doorSlotKey(chosen)] = true;' 'Desk secret passages reserve door slots like normal doors'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'secretPassageDoorSegmentsByRoom' 'Desk secret passage endpoints are included in per-room door segments'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'segments.push(segment);' 'Desk room contouring and lamps treat secret passage endpoints as doors'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "clickedTitle.getAttribute('data-desk-room-title') || '') === state.currentRoom" 'Desk map room title clicks are reserved only for current-room inline rename double-clicks'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'if (titleRoomPath === state.currentRoom)' 'Desk map title double-clicks only rename the current room'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'beginMapRoomRename(titleRoomPath);' 'Desk map current title double-click opens inline rename'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "topologyToggle('room_topology'" 'Desk room properties expose a subroom topology toggle'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" '<span>Connected room</span>' 'Desk room topology connected label is singular'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" '<span>Contained subdivision</span>' 'Desk contained topology label is singular'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" '<span>Subroom</span>' 'Desk room properties use singular Subroom field labels'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function renderMapSteady(data)' 'Desk has a steady map render path for UI-only map state changes'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function refreshMapSteadyFrom(data, fallbackRoom)' 'Desk has a steady map refresh path for map API actions'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'state.secretPassageSource = clickedRoom;' 'Desk secret passage selection marks the first chosen room'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'renderMapSteady(state.data);' 'Desk secret passage room selection keeps the open map steady'
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'refreshMapSteadyFrom(data);' 'Desk secret passage creation keeps the open map steady after API refresh'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '#desk-page-root :is(' 'Desk page has a broad scoped button reset'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '-webkit-appearance: none !important;' 'Desk button reset clears WebKit native button painting'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'border-image: none !important;' 'Desk button reset clears native border-image artifacts'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '#desk-page-root :is(.desk-status, .desk-compose-paper-picker)' 'Desk button-like wrappers no longer carry drop shadows'
...
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-map-properties-title {' 'border: 0 !important;' 'Desk room properties title removes inherited underline styling'
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '.desk-map-properties {' 'overflow: hidden;' 'Desk room properties panel clips controls inside its right edge'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "activateMapRoom(roomLink.getAttribute('data-desk-room-link') || '')" 'Desk map single-clicking another room title travels through the enclosing room link'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'previewedMapNavigation: true' 'Desk room clicks do not replay the same zoom animation after refresh'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" "if (state.mapZoomMode === 'room')" 'Desk only starts room dragging while the map is zoomed out'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'grid-template-columns: var(--desk-notebook-margin) var(--desk-notebook-line) minmax(0, 1fr);' 'Desk checklist text column does not reserve hidden row-action width'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'right: calc(-1 * var(--desk-notebook-right-margin) + 0.18rem);' 'Desk checklist row actions sit in the notebook right margin instead of shrinking text'
assert_file_not_contains "$ROOT_DIR/site/static/desk-page.css" 'padding-right: 4.6rem;' 'Desk checklist row actions do not reflow task text on hover'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" 'transition: none !important;' 'Desk checklist upvote matches Reading List instant color changes'
assert_file_contains "$ROOT_DIR/site/static/desk-page.css" '-webkit-text-fill-color: var(--list-vote-up-color, #2f8a5c);' 'Desk checklist upvoted arrows stay visibly green'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'function preserveOptimisticDeskVote' 'Desk checklist keeps an accepted optimistic vote if the server response is stale'
...
assert_file_contains "$ROOT_DIR/site/static/desk-page.js" 'Desk vote may still be saving.' 'Desk checklist does not regress optimistic votes on transient network failures'
...
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '#desk-page-root .desk-todo-close,' 'background: transparent !important;' 'Desk notebook close, checkbox, upvote, and menu controls keep transparent backgrounds'
assert_css_block_contains "$ROOT_DIR/site/static/desk-page.css" '#desk-page-root .desk-todo-close,' 'box-shadow: none !important;' 'Desk notebook close, checkbox, upvote, and menu controls have no drop shadows'
