...
assert_contains "$(cat "$ROOT_DIR/site/static/style.css")" '#main-content .zap-inline-host.is-compact .zap-compact-btn.zap-action-btn-primary' 'compact zap button keeps its dedicated primary styling after content resets'
assert_contains "$(cat "$ROOT_DIR/site/static/style.css")" 'background-image: linear-gradient(135deg, #7c3aed 0%, #d946ef 100%) !important;' 'compact zap button uses the magenta-purple gradient'
assert_contains "$(cat "$ROOT_DIR/site/static/style.css")" 'fill: currentColor !important;' 'compact zap icon paints white from the button text color'
...
assert_contains "$(cat "$ROOT_DIR/site/static/zap-ui.js")" 'window.CitrineNostrWeb.createSignedZapRequest' 'zap UI delegates signed NIP-57 zap requests to Citrine'
assert_contains "$(cat "$ROOT_DIR/site/static/vendor/citrine-nostr-web.js")" "['p', recipientPubkey]" 'Citrine zap request p tag uses the LNURL provider recipient pubkey'
assert_not_contains "$(cat "$ROOT_DIR/site/static/vendor/citrine-nostr-web.js")" "['p', target.recipientPubkey]" 'Citrine zap request does not use the post author pubkey as the zap recipient'
...
assert_contains "$(cat "$ROOT_DIR/src/gazeta_read/btc_rate.rs")" 'api.exchange.coinbase.com/products/BTC-USD/ticker' 'BTC/USD endpoint uses Coinbase ticker data'
