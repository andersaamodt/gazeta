...
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'video-chat-widget.js?v=20260523-event-rooms1' 'contact page loads the event-room video widget build'
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" '<script src="/static/video-chat-widget.js?v=20260523-event-rooms1" data-video-chat-widget="1"></script>' 'contact page HTML loads the video widget before contact-page hydration'
...
assert_contains "$blog_get_config_out" '"video_chat":{"participant_limit":8,"token_ttl_seconds":7200,"janus_wss":"wss://janus.example.com/janus","signaling_wss":"wss://signal.example.com/ws","public_rooms":true,"rooms":["Lobby","Office Hours"],"include_syntax":"{{video-chat}}"}' 'blog-get-config returns video calling settings and include syntax'
