...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'For Login via Nostr, install first:' 'Login recommendation heading names the login purpose explicitly'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'For Zaps, add:' 'Zap recommendation heading names the zap purpose explicitly'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "source: 'F-Droid', label: 'Download', url: 'https://f-droid.org/packages/com.greenart7c3.nostrsigner/'" 'Android login onboarding links directly to Amber on F-Droid'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "intro: 'Open the Nostr Connect link, then return here after pairing.'" 'Android phone signer guidance avoids repeating the Amber recommendation'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "source: 'Aurora', label: 'Download', url: 'https://auroraoss.com/downloads/AuroraStore/'" 'Android zap onboarding falls back to Aurora Store download when direct F-Droid app pages are unavailable'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.auth-login-onboarding' 'Login onboarding section has deliberate modal styling'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "els.authNip46Open.textContent = 'Open Nostr Connect'" 'phone signer open link hides the raw URI behind button text'
