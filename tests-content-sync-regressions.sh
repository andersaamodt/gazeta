...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'insertBefore(node, tabFrame.nextSibling)' 'Sign-in modal fallback message is inserted below the active sign-in controls'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "summary: 'Login'" 'Login recommendation group is concise'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'Use a Nostr event signer to log in to andersaamodt.com.' 'Login recommendation explains why a signer is needed'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "summary: 'Zaps'" 'Zap recommendation group is concise'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'Zaps are small Lightning payments. To send them, use a Nostr client and a Lightning wallet.' 'Zap recommendation explains Lightning wallet and Nostr client needs'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" "setNip46Diagnostics('Waiting for signer', 'waiting')" 'phone signer waiting state uses compact user-facing wording'
