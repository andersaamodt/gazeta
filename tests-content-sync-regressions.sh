...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'F-Droid, Aurora Store (Play Store)' 'Android zap onboarding lists F-Droid and Aurora Store sources'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'Damus or Nostur' 'iOS zap onboarding has an honest iOS client path'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" '<strong>Manual</strong><span>Challenge JSON</span>' 'Manual challenge fallback is labelled as an advanced manual path'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'nav-auth.js?v=20260430-zap-onboarding1' 'nav auth script URL is cache-busted after zap onboarding recommendations'
