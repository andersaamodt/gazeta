...
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" "blog_nostr_list_page_js_version='20260522-row-menu-overlay1'" 'list page script cache buster tracks row menu overlay rendering'
...
assert_file_contains "$ROOT_DIR/cgi/blog-open-post" '<div class="post-page-menu" hidden>' 'single post server menu is hidden until auth and reader actions are resolved'
assert_file_contains "$ROOT_DIR/cgi/blog-open-post" 'data-post-page-admin-action role="menuitem" hidden>Edit post...' 'single post server edit action is hidden until admin auth resolves'
assert_file_contains "$ROOT_DIR/cgi/blog-open-post" 'data-post-page-admin-action role="menuitem" hidden>Add to list...' 'single post server add-to-list action is hidden until admin auth resolves'
assert_file_contains "$ROOT_DIR/cgi/blog-open-post" 'data-post-page-admin-action role="menuitem" hidden>Delete post...' 'single post server delete action is hidden until admin auth resolves'
