...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'insertBefore(node, tabs)' 'Sign-in modal fallback message is inserted above the tabs'
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'Browser signer</button>' 'Sign-in modal labels browser signer path without implying registration is separate'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'Manual challenge</button>' 'Manual challenge fallback is labelled as an advanced manual path'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'nav-auth.js?v=20260429-amber-diag1' 'nav auth script URL is cache-busted after Amber pairing diagnostics fix'
assert_file_contains "$SITE_SOURCE_ROOT/includes/head.html" 'zap-ui.js?v=20260429-zap-amber1' 'zap UI script URL is cache-busted after Amber zap fix'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'function buildAmberIntentUri' 'Nostr Connect exposes an Android Amber intent fallback'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'package=com.greenart7c3.nostrsigner' 'Amber intent fallback targets Amber on Android'
assert_file_not_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'window.location.href = href;' 'Nostr Connect lets native anchor dispatch open Amber instead of intercepting clicks'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'navigator.clipboard.writeText(uri)' 'Nostr Connect link can be copied for manual Amber import'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'signerPubkey: normalizePubkeyHex(state.nip46.signerPubkey' 'Nostr Connect stores the Amber transport pubkey for later zap signing'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'accountPubkey: normalizePubkeyHex(state.nip46.accountPubkey' 'Nostr Connect stores the Amber account pubkey separately from the transport key'
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" 'function getNip46AccountPubkey()' 'shared zap signer asks Amber for the actual account pubkey'
...
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'auth-nip46-amber-open' 'phone signer panel includes Android Amber intent fallback link'
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'auth-nip46-copy' 'phone signer panel includes copy link fallback'
assert_file_contains "$SITE_SOURCE_ROOT/includes/nav.md" 'auth-nip46-reset' 'phone signer panel includes fresh-link recovery action'
