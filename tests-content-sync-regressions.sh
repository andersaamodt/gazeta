...
assert_file_contains "$SITE_SOURCE_ROOT/static/style.css" '.contact-section-heading::after' 'contact page section headings draw a line across the remaining width'
...
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" "blog_nostr_contact_page_js_version='20260523-contact-headings1'" 'contact page cache buster tracks contact heading rendering'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/nav-auth.js" '20260522-blog-filter-right1' 'auth theme switcher uses current blog filter placement theme cache buster'
