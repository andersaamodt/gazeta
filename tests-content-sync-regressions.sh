...
assert_file_contains "$ROOT_DIR/cgi/blog-manage-zaps" 'Provision Bitcoin, Core Lightning, and the Lightning Address endpoint from Headquarters.' 'zaps cgi reports external software management boundary'
assert_file_contains "$ROOT_DIR/cgi/blog-manage-zaps" '"effective_lud16"' 'zaps cgi runtime emits effective Lightning Address'
assert_file_contains "$ROOT_DIR/wizardry-server-requirements.conf" 'bitcoin_node=required' 'server requirements declare the Bitcoin node as required'
assert_file_contains "$ROOT_DIR/wizardry-server-requirements.conf" 'lightning_node=required' 'server requirements declare the Lightning node as required'
assert_file_contains "$ROOT_DIR/wizardry-server-requirements.conf" 'zap_endpoint=required' 'server requirements declare the Lightning Address endpoint as required'
