...
assert_file_contains "$ROOT_DIR/includes/head.html" 'scheduleRouteLoadFailure();' 'document head defers route failure panel to avoid transient flash during hydration'
assert_file_contains "$ROOT_DIR/includes/head.html" "window.addEventListener('blog-page-initial-content-ready'" 'document head cancels route failure fallback once initial content paints'
...
assert_file_contains "$ROOT_DIR/static/list-page.js" "document.getElementById('oeuvre-root')" 'list runtime still supports legacy oeuvre root id'
...
assert_file_contains "$ROOT_DIR/static/list-page.js" "var defaultMarker = (slug === 'list' || slug === 'oeuvre') ? slug : '';" 'new list entries default marker preserves legacy oeuvre slug compatibility'
...
assert_file_contains "$ROOT_DIR/static/style.css" 'html.app-hydrating #icon-gallery-root,' 'hydration gate hides icon gallery shell until runtime is ready'
assert_file_contains "$ROOT_DIR/static/style.css" 'html.app-hydrating #oeuvre-root,' 'hydration gate still supports legacy oeuvre shell ids'
assert_file_contains "$ROOT_DIR/includes/nav.md" "document.getElementById('icon-gallery-root')" 'nav hydration detection treats icon-gallery pages as dynamic roots'
assert_file_contains "$ROOT_DIR/includes/nav.md" "document.getElementById('oeuvre-root')" 'nav hydration detection preserves legacy oeuvre shell support'
assert_file_contains "$ROOT_DIR/static/nav-auth.js" "document.getElementById('icon-gallery-root')" 'nav-auth waits for icon-gallery content before nav reveal'
assert_file_contains "$ROOT_DIR/static/nav-auth.js" "document.getElementById('oeuvre-root')" 'nav-auth waits for legacy oeuvre content before nav reveal'
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" "grep -q 'id=\"oeuvre-root\"' \"\$file\"" 'nostr page type detection still classifies legacy oeuvre shells as list templates'
assert_file_contains "$ROOT_DIR/cgi/blog-nostr-pages-common.sh" 'if [ "$slug" = "oeuvre" ]; then' 'nostr page type resolver preserves legacy oeuvre slug alias'
assert_file_contains "$ROOT_DIR/static/oeuvre.js" "/static/list-page.js?v=20260326-pagefix5" 'legacy oeuvre runtime alias forwards to list-page runtime'
...
assert_file_contains "$ROOT_DIR/static/style.css" '.blog-post-item .blog-post-byline {' 'blog list cards expose dedicated byline row for author/date alignment'
assert_file_contains "$ROOT_DIR/static/style.css" '.blog-post-item .blog-post-footer {' 'blog list cards expose dedicated footer row for tags/comments alignment'
assert_file_contains "$ROOT_DIR/static/blog-page.js" '<div class="blog-post-byline"><div class="post-author">' 'blog list card renders author/date byline row'
assert_file_contains "$ROOT_DIR/static/blog-page.js" '<div class="blog-post-footer">' 'blog list card renders tags/comments footer row'
assert_file_contains "$ROOT_DIR/cgi/blog-index" 'class="post-item blog-post-item"' 'cgi blog index mirrors blog list card class for shared list styling'
...
assert_file_contains "$ROOT_DIR/pages/admin.md" '.noster-relay-ssl.is-ok {' 'nostr runtime UI includes relay SSL indicator style'
assert_file_contains "$ROOT_DIR/static/admin.js" 'data-noster-action="activate_relay_url"' 'nostr runtime UI exposes relay URL setup action button'
assert_file_contains "$ROOT_DIR/static/admin.js" '<strong>Relay URL</strong>' 'nostr runtime UI shows relay URL line item'
