...
assert_file_contains "$ROOT_DIR/pages/admin.md" 'id="compose-shortform-meter"' 'admin compose renders shortform counter row'
assert_file_contains "$ROOT_DIR/pages/admin.md" 'id="btn-compose-shortform-limit"' 'admin compose shortform counter toggle button'
assert_file_contains "$ROOT_DIR/pages/admin.md" 'id="compose-shortform-limit-input"' 'admin compose shortform custom limit input'
...
assert_file_contains "$ROOT_DIR/static/admin.js" "els.composeShortformLimitButton.addEventListener('dblclick'" 'admin shortform limit supports custom edit mode'
...
assert_file_contains "$ROOT_DIR/static/admin.js" "title: postType === 'shortform' ? '' : titleValue," 'admin compose sends empty title for shortform payloads'
assert_file_contains "$ROOT_DIR/static/admin.js" 'titleRow.hidden = !showTitleField;' 'admin compose hides title row for shortform'
...
assert_file_contains "$ROOT_DIR/static/blog-page.js" "publishDestination: 'local_only'," 'blog compose defaults destination to local only'
assert_file_contains "$ROOT_DIR/static/blog-page.js" "return 'local_only';" 'blog compose destination normalizer defaults to local only'
...
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'function setComposeShortformLimit(raw, opts) {' 'blog compose supports editable shortform limits'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'data-compose-action="shortform-limit-toggle"' 'blog compose renders shortform counter toggle'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'data-compose-field="shortform-limit"' 'blog compose renders shortform custom limit input'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'function composeTextareaRows(postType) {' 'blog compose calculates per-type textarea height'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'if (type === '\''shortform'\'') {' 'blog compose has shortform-specific textarea sizing'
assert_file_contains "$ROOT_DIR/static/blog-page.js" "if (actionName === 'audio-record-toggle')" 'blog compose exposes audio recorder action'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'new MediaRecorder(stream' 'blog compose uses MediaRecorder for audio-note mode'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'function preferredComposeAudioMimeType() {' 'blog compose picks supported audio recorder mime types'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'Press Start Recording to request microphone access.' 'blog compose audio mode gives explicit permission prompt guidance'
assert_file_contains "$ROOT_DIR/static/blog-page.js" "root.classList.remove('blog-camera-mode');" 'camera mode no longer hides entire page content'
...
assert_file_contains "$ROOT_DIR/static/blog-page.js" "title: fields.postType === 'shortform' ? '' : fields.title.trim()," 'blog compose sends empty title for shortform payloads'
assert_file_contains "$ROOT_DIR/static/blog-page.js" "blog-compose-title-row\"' + (showTitleField ? '' : ' hidden aria-hidden=\"true\"')" 'blog compose hides title row when shortform is selected'
...
assert_file_contains "$ROOT_DIR/static/post-context.js" "title: postType === 'shortform' ? '' : String(inlineEditState.title || '').trim()," 'inline post editor sends empty title for shortform payloads'
assert_file_contains "$ROOT_DIR/static/post-context.js" "blog-compose-title-row\"' + (showTitleField ? '' : ' hidden aria-hidden=\"true\"')" 'inline post editor hides title row when shortform is selected'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'data-compose-action="capture-photo"' 'blog compose camera mode has capture action'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'data-compose-action="open-mode-picker"' 'blog compose mode panels provide picker action'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'data-compose-mode-target="upload-media"' 'blog compose upload-media panel opens media picker'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'data-compose-mode-target="attachment"' 'blog compose attachment panel opens file picker'
...
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'function composeCameraOverlayHtml() {' 'blog compose defines fullscreen camera overlay renderer'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'data-compose-camera-fullscreen-preview' 'blog compose camera overlay renders fullscreen video preview'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'function openComposeCameraCapture(options) {' 'blog compose has camera fullscreen entrypoint'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'state.compose.cameraFullscreen = true;' 'blog compose enables fullscreen camera mode'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'state.compose.cameraReturnToChooser = !!opts.returnToChooser;' 'blog compose tracks whether cancel should return to chooser'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'function closeComposeCameraCapture(options) {' 'blog compose has camera fullscreen exitpoint'
assert_file_contains "$ROOT_DIR/static/blog-page.js" "if (state.compose.cameraFullscreen && event.key === 'Escape') {" 'blog compose allows escape to exit fullscreen camera'
assert_file_contains "$ROOT_DIR/static/blog-page.js" "var forcePreview = postType === 'capture-media' || postType === 'upload-media';" 'blog compose forces preview visibility for media post types'
assert_file_contains "$ROOT_DIR/static/blog-page.js" "'<div class=\"blog-compose-islands\">' + composeCardHtml + previewCardHtml + '</div>'" 'blog compose renders wide preview as second island'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'blog-compose-preview-card' 'blog compose defines dedicated preview island card'
assert_file_contains "$ROOT_DIR/static/blog-page.js" 'aria-pressed="' + previewTogglePressed + '"' 'blog compose preview toggle exposes pressed state'
...
assert_file_contains "$ROOT_DIR/static/style.css" '.blog-compose-shortform-meter {' 'blog style defines shortform counter row'
assert_file_contains "$ROOT_DIR/static/style.css" '.blog-compose-card .compose-audio-status {' 'blog style defines audio recorder status text'
assert_file_not_contains "$ROOT_DIR/static/style.css" '#blog-page-root.blog-camera-mode #blog-post-list' 'camera mode does not hide post list content'
...
assert_file_contains "$ROOT_DIR/static/style.css" '.blog-compose-islands {' 'blog style defines two-island compose/preview layout'
assert_file_contains "$ROOT_DIR/static/style.css" '.blog-compose-preview-card {' 'blog style defines separate preview island card'
assert_file_contains "$ROOT_DIR/static/style.css" '.blog-compose-card .blog-compose-preview-toggle[aria-pressed="true"]' 'blog style highlights active preview toggle'
assert_file_contains "$ROOT_DIR/static/style.css" 'body.blog-camera-mode-active {' 'blog style locks page scroll in fullscreen camera mode'
assert_file_contains "$ROOT_DIR/static/style.css" '.blog-compose-camera-overlay {' 'blog style defines fullscreen camera overlay shell'
