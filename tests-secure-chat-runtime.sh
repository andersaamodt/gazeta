...
assert_file_contains "$ROOT_DIR/site/static/simplex-web-default-chat.js" 'secure-chat-emoji-picker" emoji-version="17.0"' 'Secure Chat emoji picker renders the standard emoji web component'
...
assert_file_contains "$ROOT_DIR/site/static/contact-page.js" 'insertSecureChatEmoji(event.detail.unicode' 'Secure Chat inserts selected emoji into the live compose textarea'
...
assert_file_contains "$ROOT_DIR/site/static/style.css" '.secure-chat-emoji-recent-grid' 'Secure Chat emoji picker displays a Recently Used section'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/simplex-web-transport.js" 'SimplexWebTransport' 'shared simplex-web transport is vendored into nostr-blog'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'data-secure-chat-action="start">Start Chat</button>' 'contact page renders a Start Chat gate for logged-in users'
assert_file_contains "$SITE_SOURCE_ROOT/static/simplex-web-default-chat.js" 'data-secure-chat-action="start">Start Chat</button>' 'shared secure chat renderer renders a Start Chat gate for logged-in users'
...
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'renderSecureChatPanel() + managedLightningNoteHtml()' 'contact page renders Secure Chat above contact methods'
