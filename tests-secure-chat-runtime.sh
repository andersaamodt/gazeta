...
assert_file_contains "$ROOT_DIR/cgi/blog-secure-chat-state" 'blog-secure-chat-fast-state.js' 'secure chat browser state reads accepted local messages without waiting for SimpleX service health'
assert_file_contains "$ROOT_DIR/cgi/blog-secure-chat-fast-state.js" "status: 'file-state'" 'fast browser state reports file-backed state status'
...
assert_file_contains "$ROOT_DIR/cgi/blog-secure-chat-state" 'blog-secure-chat-fast-state.js' 'state endpoint prints file-backed json responses explicitly'
