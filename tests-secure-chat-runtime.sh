...
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'profile: secureChatBrowserProfile()' 'contact page sends the site username as the browser-native SimpleX profile'
assert_file_contains "$SITE_SOURCE_ROOT/static/contact-page.js" 'owner-native-inline-v4-' 'contact page isolates browser-native SimpleX contacts by owner link and username'
...
assert_file_contains "$ROOT_DIR/cgi/blog-secure-chat-service.js" 'const parsedAttachment = parseAttachmentMarker(row.text || ' 'owner-direct Owl export parses SimpleX attachment envelopes'
