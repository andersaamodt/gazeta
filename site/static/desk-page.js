(function () {
  ...
  function applyPresenceValues(rooms) {
    ...
    root.querySelectorAll('[data-desk-door-from][data-desk-door-to]').forEach(function (node) {
  ...
  function roomDeleteDisabledReason(room) {
    if (!room || !String(room.path || '')) {
      return 'Office cannot be deleted.';
    }
    if (room.can_delete_room) {
      return '';
    }
    return String(room.delete_room_disabled_reason || 'Room is not empty.');
  }

  function renderRoomDeleteControl(room) {
    var path = String(room && room.path || '');
    var reason = roomDeleteDisabledReason(room);
    return '<div class="desk-map-room-delete-control">' +
      '<button type="button" class="desk-map-room-delete-btn" data-desk-delete-room="' + escapeHtml(path) + '"' + (reason ? ' disabled' : '') + '>Delete Room</button>' +
  ...
  function renderGreenbelt(cells, occupied, unitW, unitH, color) {
    if (!cells.length) return '';
    var green = /^#[0-9a-f]{6}$/i.test(color || '') ? color : '#9fac63';
    var edges = [];
    var corners = {};
    var bandRadius = 30;
    ...
    return '<g class="desk-map-greenbelt" aria-hidden="true">' +
      '<defs><linearGradient id="desk-map-greenbelt-gradient-north" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="' + green + '" stop-opacity="0"></stop><stop offset="0.34" stop-color="' + green + '" stop-opacity="0.13"></stop><stop offset="0.72" stop-color="' + green + '" stop-opacity="0.58"></stop><stop offset="1" stop-color="' + green + '" stop-opacity="1"></stop></linearGradient><linearGradient id="desk-map-greenbelt-gradient-south" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="' + green + '" stop-opacity="1"></stop><stop offset="0.28" stop-color="' + green + '" stop-opacity="0.58"></stop><stop offset="0.66" stop-color="' + green + '" stop-opacity="0.13"></stop><stop offset="1" stop-color="' + green + '" stop-opacity="0"></stop></linearGradient><linearGradient id="desk-map-greenbelt-gradient-east" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="' + green + '" stop-opacity="1"></stop><stop offset="0.28" stop-color="' + green + '" stop-opacity="0.58"></stop><stop offset="0.66" stop-color="' + green + '" stop-opacity="0.13"></stop><stop offset="1" stop-color="' + green + '" stop-opacity="0"></stop></linearGradient><linearGradient id="desk-map-greenbelt-gradient-west" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="' + green + '" stop-opacity="0"></stop><stop offset="0.34" stop-color="' + green + '" stop-opacity="0.13"></stop><stop offset="0.72" stop-color="' + green + '" stop-opacity="0.58"></stop><stop offset="1" stop-color="' + green + '" stop-opacity="1"></stop></linearGradient><radialGradient id="desk-map-greenbelt-gradient-corner-nw" cx="100%" cy="100%" r="100%"><stop offset="0.26" stop-color="' + green + '" stop-opacity="1"></stop><stop offset="0.54" stop-color="' + green + '" stop-opacity="0.48"></stop><stop offset="0.8" stop-color="' + green + '" stop-opacity="0.11"></stop><stop offset="1" stop-color="' + green + '" stop-opacity="0"></stop></radialGradient><radialGradient id="desk-map-greenbelt-gradient-corner-ne" cx="0%" cy="100%" r="100%"><stop offset="0.26" stop-color="' + green + '" stop-opacity="1"></stop><stop offset="0.54" stop-color="' + green + '" stop-opacity="0.48"></stop><stop offset="0.8" stop-color="' + green + '" stop-opacity="0.11"></stop><stop offset="1" stop-color="' + green + '" stop-opacity="0"></stop></radialGradient><radialGradient id="desk-map-greenbelt-gradient-corner-se" cx="0%" cy="0%" r="100%"><stop offset="0.26" stop-color="' + green + '" stop-opacity="1"></stop><stop offset="0.54" stop-color="' + green + '" stop-opacity="0.48"></stop><stop offset="0.8" stop-color="' + green + '" stop-opacity="0.11"></stop><stop offset="1" stop-color="' + green + '" stop-opacity="0"></stop></radialGradient><radialGradient id="desk-map-greenbelt-gradient-corner-sw" cx="100%" cy="0%" r="100%"><stop offset="0.26" stop-color="' + green + '" stop-opacity="1"></stop><stop offset="0.54" stop-color="' + green + '" stop-opacity="0.48"></stop><stop offset="0.8" stop-color="' + green + '" stop-opacity="0.11"></stop><stop offset="1" stop-color="' + green + '" stop-opacity="0"></stop></radialGradient></defs>' +
      gradientEdges +
      gradientCorners +
  ...
  function doorPresenceVars(fromRoom, toRoom, presence) {
    if (fromRoom == null || toRoom == null) return { attrs: '', style: '' };
    var from = String(fromRoom || '');
    var to = String(toRoom || '');
    var fromPresence = clampPresence(presence && presence[from]);
  ...
  function renderDoorGlyph(x, y, side, isSecret, isLastEntered, spill, options) {
    var secretClass = isSecret ? ' desk-map-secret-door' : '';
    var doorwayOnly = Boolean(options && options.doorwayOnly);
    var doorwayClass = doorwayOnly ? ' is-doorway-only' : '';
    var enteredClass = isLastEntered ? ' is-last-entered' : '';
    var spillAttrs = spill && spill.attrs ? spill.attrs : '';
    var spillStyle = spill && spill.style ? spill.style : '';
    var open = 13;
    var leaf = 24;
    var label = isSecret
    ...
    if (side === 'east' || side === 'west') {
      ...
      return '<g class="desk-map-door' + secretClass + doorwayClass + enteredClass + '"' + spillAttrs + spillStyle + '><path class="desk-map-door-spill-fan to-room spill-' + doorFanDirection(side, true) + '" d="' + doorFanPath(x, y, side, true) + '"></path><path class="desk-map-door-spill-fan from-room spill-' + doorFanDirection(side, false) + '" d="' + doorFanPath(x, y, side, false) + '"></path><path class="' + (doorwayOnly ? 'desk-map-doorway-gap' : 'desk-map-door-gap') + '" d="M ' + x + ' ' + (y - open) + ' V ' + (y + open) + '"></path>' + (doorwayOnly ? '' : '<path class="desk-map-door-leaf" d="M ' + x + ' ' + (y + open) + ' L ' + xLeaf + ' ' + (y + open) + '"></path><path class="desk-map-door-swing" d="M ' + xLeaf + ' ' + (y + open) + ' Q ' + xLeaf + ' ' + y + ' ' + x + ' ' + (y - open) + '"></path>') + label + '</g>';
    }
    var yDirection = side === 'south' ? 1 : -1;
    var yLeaf = y + yDirection * leaf;
    return '<g class="desk-map-door' + secretClass + doorwayClass + enteredClass + '"' + spillAttrs + spillStyle + '><path class="desk-map-door-spill-fan to-room spill-' + doorFanDirection(side, true) + '" d="' + doorFanPath(x, y, side, true) + '"></path><path class="desk-map-door-spill-fan from-room spill-' + doorFanDirection(side, false) + '" d="' + doorFanPath(x, y, side, false) + '"></path><path class="' + (doorwayOnly ? 'desk-map-doorway-gap' : 'desk-map-door-gap') + '" d="M ' + (x - open) + ' ' + y + ' H ' + (x + open) + '"></path>' + (doorwayOnly ? '' : '<path class="desk-map-door-leaf" d="M ' + (x - open) + ' ' + y + ' L ' + (x - open) + ' ' + yLeaf + '"></path><path class="desk-map-door-swing" d="M ' + (x - open) + ' ' + yLeaf + ' Q ' + x + ' ' + yLeaf + ' ' + (x + open) + ' ' + y + '"></path>') + label + '</g>';
  }
  ...
  function renderDoor(door, layout, unitW, unitH, presence, roomsByPath) {
    var from = layout[door.from];
    var to = layout[door.to];
    if (!from || !to) return '';
    var isLastEntered = isLastEnteredDoor(door);
    var spill = doorPresenceVars(door.from, door.to, presence);
    var fromRoom = roomsByPath && roomsByPath[String(door.from || '')];
    var toRoom = roomsByPath && roomsByPath[String(door.to || '')];
    var renderDoorwayOnly = roomIsOutdoor(fromRoom) && roomIsOutdoor(toRoom);
    if (Number.isFinite(door.x) && Number.isFinite(door.y)) {
      return renderDoorGlyph(door.x * unitW, door.y * unitH, door.side, false, isLastEntered, spill, { doorwayOnly: renderDoorwayOnly });
    }
    var x;
    var y;
    if (door.side === 'east' || door.side === 'west') {
      ...
      return renderDoorGlyph(x, y, door.side, false, isLastEntered, spill, { doorwayOnly: renderDoorwayOnly });
    }
  ...
  function renderMap(data) {
    ...
    var zoomLabel = state.mapZoomMode === 'room' ? 'Show whole map' : 'Zoom to current room';
    var gridPadX = fullViewBox.w;
    var gridPadY = fullViewBox.h;
    var gridBox = {
      x: fullViewBox.x - gridPadX,
      y: fullViewBox.y - gridPadY,
      w: fullViewBox.w + gridPadX * 2,
      h: fullViewBox.h + gridPadY * 2
    };
    return '<section class="desk-mode-panel desk-map-panel' + (state.closingMode === 'map' || state.closingMap ? ' is-closing' : '') + (state.suppressMapAnimation ? ' is-steady' : '') + '" aria-label="Room map" style="--desk-map-aspect:' + escapeHtml(mapAspect) + ';--desk-greenbelt-color:' + escapeHtml(mapGreen) + ';">' +
  ...
  function openMode(mode) {
    ...
    var keepPaperMap = previousMode === 'map' || ((previousMode === 'todo' || previousMode === 'compose') && state.paperMapVisible);
    clearModeCloseTimer();
  ...
  function closeTodoMode() {
    ...
    clearModeCloseTimer();
    playSound(deskSounds.book);
    var returnToMap = state.paperMapVisible;
  ...
  root.addEventListener('click', function (event) {
    ...
    var deleteRoom = event.target.closest('[data-desk-delete-room]');
    if (deleteRoom) {
      event.preventDefault();
      if (deleteRoom.disabled) {
        return;
      }
      var deletePath = deleteRoom.getAttribute('data-desk-delete-room') || '';
      api('delete-room', {
        room: deletePath
      }).then(function (data) {
        if (data && data.success !== false) {
