...
function xftpWebErrorText(error) {
  var value = error instanceof Uint8Array ? error : toBytes(error || new Uint8Array(), 'XFTP web error');
  return String.fromCharCode(...value).trim();
}

function xftpWebErrorIsRetriable(error) {
  var code = String(error && error.code || '');
  var message = String(error && error.message || error || '');
  return code === 'XFTP_WEB_SESSION' ||
    /XFTP web transmission failed: (SESSION|HANDSHAKE)\b/.test(message) ||
    /\bERR (SESSION|HANDSHAKE|AUTH|CMD NO_AUTH|CMD AUTH)\b/.test(message);
}
...
export async function connectBrowserXftpWebClient(options = {}) {
  ...
  client.reconnect = async function reconnect() {
    var next = await connectBrowserXftpWebClient({
      ...options,
      url,
      keyHash,
      fetchImpl,
    ...
    client.version = next.version;
    client.sessionId = next.sessionId;
    client.handshake = next.handshake;
    client.security = next.security;
    return client;
  };
  return client;
}
...
export async function sendXftpWebCommand(client, options = {}) {
  try {
    return await sendXftpWebCommandOnce(client, options);
  } catch (error) {
    if (options.retry === false || !xftpWebErrorIsRetriable(error) || !client || typeof client.reconnect !== 'function') {
      throw error;
    }
    await client.reconnect();
    return sendXftpWebCommandOnce(client, { ...options, retry: false });
  }
}

async function sendXftpWebCommandOnce(client, options = {}) {
  ...
  if (tx.response.type === 'ERR') fail('XFTP_WEB_SERVER', 'XFTP web server returned ERR ' + xftpWebErrorText(tx.response.error));
  return {
    ...tx,
