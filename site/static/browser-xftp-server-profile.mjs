...
function safeEndpoint(raw, transport) {
  ...
  if (parsed.username || parsed.password) {
    fail('XFTP_SERVER_PROFILE_URL', 'browser XFTP endpoint URL must not contain credentials');
  }
  parsed.hash = '';
  return parsed;
}
...
function safeOrigin(raw) {
  ...
  if (parsed.username || parsed.password) {
    fail('XFTP_SERVER_PROFILE_ORIGIN', 'allowed origin must not contain credentials');
  }
