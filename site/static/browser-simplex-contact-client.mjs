...
const NATIVE_DIRECT_FILE_CHUNK_SIZE = 10000;
...
function nativeInvitationUriFromQueue(queue, x3dhKey1, x3dhKey2, options = {}) {
  ...
  if (!q.server) fail('SIMPLEX_CONTACT_NATIVE_FILE', 'native file invitation queue requires a server');
  ...
  var e2e = 'v=' + encodeURIComponent(String(options.nativeE2EVersionRange || options.e2eVersionRange || '2')) +
    '&x3dh=' + [
...
function chatMessageFromNativeConnInfo(bytes, label = 'native file connection info') {
  var decoded = parseJsonBytes(bytes, label) || {};
  if (!decoded.event || typeof decoded.event !== 'string') fail('SIMPLEX_CONTACT_NATIVE_FILE', label + ' is missing an event');
  return decoded;
}

function encodeNativeChatEvent(event, params = {}) {
  return utf8Bytes(JSON.stringify({ event, params }));
}

function encodeNativeDirectFileChunk(chunkNo, chunkBytes) {
  var n = Math.max(1, Math.floor(Number(chunkNo || 1) || 1));
  return concatBytes(utf8Bytes('F'), encodeWord32(n), toBytes(chunkBytes, 'native direct file chunk'));
}

function splitNativeDirectFile(bytes, chunkSize) {
  var input = toBytes(bytes, 'native direct file bytes');
  var size = Math.max(1, Math.floor(Number(chunkSize || NATIVE_DIRECT_FILE_CHUNK_SIZE) || NATIVE_DIRECT_FILE_CHUNK_SIZE));
  var chunks = [];
  for (var offset = 0; offset < input.length; offset += size) chunks.push(input.slice(offset, Math.min(input.length, offset + size)));
...
export class BrowserSimplexContactClient {
  ...
  async sendFile(id, bytes, options = {}) {
    ...
    if (contact && (contact.nativeAgentProfile || (ratchet && ratchet.nativeAgentProfile))) {
      return this.sendNativeDirectFile(contact, null, bytes, options);
    }
  ...
  async sendNativeDirectFile(contact, _file, bytes, options = {}) {
    requireState(contact, [CONTACT_STATE_ACTIVE]);
    var fileBytes = toBytes(bytes, 'native direct file bytes');
    var sharedMsgId = base64UrlPadded(options.sharedMsgId || randomNonce24());
    var fileName = nativeFileName({ manifest: { fileName: options.name || options.fileName || options.clientMessageId || 'file' } }, 'file');
    var fileRef = String(options.clientMessageId || sharedMsgId).replace(/=+$/g, '');
    var fileQueueId = contact.id + ':file:' + safeMessageRef(fileRef, 'native file message ref');
    var fileQueue = await this.client.createQueue({
      ...options,
      label: fileQueueId,
      queueMode: options.fileQueueMode || 'messaging',
      corrId: options.fileQueueCorrId || ('file-new-' + Date.now())
    });
    var recipientX3dhKey1 = options.fileRecipientX3dhKey1 || generateX448KeyPair(options.fileRecipientX3dhSeed1);
    var recipientX3dhKey2 = options.fileRecipientX3dhKey2 || generateX448KeyPair(options.fileRecipientX3dhSeed2);
    var fileConnReq = nativeInvitationUriFromQueue(fileQueue, recipientX3dhKey1, recipientX3dhKey2, options);
    this.store.saveQueue(fileQueueId, fileQueue);
    this.store.save('nativeFile', fileQueueId, {
      contactId: contact.id,
      fileName,
      ...
      recipientX3dhKey1,
      recipientX3dhKey2,
      chunkSize: options.fileChunkSize || NATIVE_DIRECT_FILE_CHUNK_SIZE,
      createdAt: nowIso()
    });
    var inviteMessage = {
      v: '1',
      ...
      params: {
        ...
        file: { fileName, fileSize: fileBytes.length, fileConnReq }
      }
    };
    var invite = await this.sendNativeChatBody(contact, encodeNativeChatJson(inviteMessage), {
      ...options,
    ...
    if (options.waitForNativeFileAccept === true) {
      transfer = await this.acceptAndSendNativeDirectFile(contact, fileQueueId, fileBytes, {
        ...options,
        timeoutMs: options.nativeFileAcceptTimeoutMs || options.fileAcceptTimeoutMs || options.timeoutMs || 30000
      });
    }
    return { ...invite, file: transfer, nativeDirectFile: transfer };
  }

  async acceptAndSendNativeDirectFile(contact, fileQueueId, bytes, options = {}) {
    var fileState = this.store.load('nativeFile', fileQueueId);
    if (!fileState) fail('SIMPLEX_CONTACT_NATIVE_FILE', 'native direct file state is missing');
    var fileQueue = options.fileQueue || this.store.loadQueue(fileQueueId);
    if (!fileQueue) fail('SIMPLEX_CONTACT_NATIVE_FILE', 'native direct file queue is missing');
    var received = await this.client.receiveQueueMessage(fileQueue, options);
    var decryptedBody = decryptRcvMessageBody({
      serverDhSecret: fileQueue.serverDhSecret,
      msgId: received.message.msgId,
      encryptedBody: received.message.body
    });
    var envelope = parseClientMessageEnvelope(decryptedBody.body);
    if (!envelope.publicHeader.e2ePubDhKey) fail('SIMPLEX_CONTACT_NATIVE_FILE', 'native file accept is missing sender E2E DH key');
    var senderDh = decodePublicKeyDer(envelope.publicHeader.e2ePubDhKey);
    var inboundShared = x25519SharedSecret(fileQueue.rcvDhKey.secretKey, senderDh.rawPublicKey);
    var decrypted = decryptClientMessageEnvelope({ sharedSecret: inboundShared, envelope: decryptedBody.body });
    if (decrypted.privateHeader.type === 'confirmation') {
      await this.client.secureQueue(fileQueue, decrypted.privateHeader.senderPublicVerifyKey, {
        ...options,
        corrId: options.fileKeyCorrId || ('file-key-' + Date.now())
      });
    }
    var nativeEnvelope = parseNativeAgentEnvelope(decrypted.body);
    if (nativeEnvelope.type !== 'confirmation') fail('SIMPLEX_CONTACT_NATIVE_FILE', 'native file accept envelope must be a confirmation');
    var e2e = parseNativeE2ERatchetParams(nativeEnvelope.e2eEncryption);
    var init = deriveNativeX3dhReceiver({
      recipientKey1: fileState.recipientX3dhKey1,
      recipientKey2: fileState.recipientX3dhKey2,
      senderKey1: e2e.key1,
      senderKey2: e2e.key2
    });
    var receivingRatchet = createNativeReceivingRatchet({
      version: e2e.version || SIMPLEX_NATIVE_E2E_VERSION,
      init,
      ownDhKey: fileState.recipientX3dhKey2
    });
    var nativePlain = decryptNativeRatchetMessage(receivingRatchet, nativeEnvelope.encConnInfo);
    var conn = parseNativeAgentConnInfo(nativePlain.plaintext);
    var acceptMessage = chatMessageFromNativeConnInfo(conn.connInfo, 'native direct file accept');
    if (acceptMessage.event !== 'x.file.acpt' || !acceptMessage.params || acceptMessage.params.fileName !== fileState.fileName) {
      fail('SIMPLEX_CONTACT_NATIVE_FILE', 'native direct file accept does not match the sent file');
    }
    var replyQueue = conn.replyQueues && conn.replyQueues[0];
    if (!replyQueue) fail('SIMPLEX_CONTACT_NATIVE_FILE', 'native direct file accept did not provide a reply queue');
    var outboundQueue = {
      server: replyQueue.server,
      sndId: replyQueue.senderId,
      senderSignKey: generateEd25519KeyPair(options.fileSenderSignSeed || options.senderSignSeed),
      recipientDhPublicKey: replyQueue.dhPublicKey,
      smpClientVersion: replyQueue.smpClientVersion || replyQueue.clientVersion || 4,
      queueMode: replyQueue.queueMode,
      nativeReplyQueue: true
    ...
    if (String(replyQueue.queueMode || '').toUpperCase() === 'M') {
      outboundQueue = prepareNativeOutboundQueue(outboundQueue, options);
      await this.client.secureSenderQueue(outboundQueue, {
        ...options,
        corrId: options.fileSkeyCorrId || ('file-skey-' + Date.now())
      });
    }
    var ratchet = nativePlain.state;
    var okMessage = encodeNativeAgentConnInfo({ type: 'info', connInfo: encodeNativeChatEvent('x.ok') });
    var okEncrypted = encryptNativeRatchetMessage(ratchet, okMessage, {
      paddedLength: options.nativeEncConnInfoLength || options.encConnInfoLength || SIMPLEX_NATIVE_ENC_CONN_INFO_LENGTH
    });
    ratchet = okEncrypted.state;
    var okEnvelope = encodeNativeAgentEnvelope({
      type: 'confirmation',
      version: options.nativeAgentVersion || 7,
      encConnInfo: okEncrypted.packet
    });
    var okClientEnvelope = encodeNativeQueueClientMessage(outboundQueue, okEnvelope, options);
    outboundQueue = okClientEnvelope.queue;
    await this.client.sendQueueMessage(outboundQueue, okClientEnvelope.body, {
      ...options,
      corrId: options.fileOkCorrId || ('file-ok-' + Date.now())
    });
    var chunks = splitNativeDirectFile(bytes, fileState.chunkSize);
    var prevMsgHash = new Uint8Array();
    for (var i = 0; i < chunks.length; i += 1) {
      var body = encodeNativeDirectFileChunk(i + 1, chunks[i]);
      var nativeMessage = encodeNativeAgentMessage({
        type: 'message',
        sndMsgId: i + 1,
        prevMsgHash,
      ...
      prevMsgHash = sha256Hash(nativeMessage);
      var encrypted = encryptNativeRatchetMessage(ratchet, nativeMessage, {
        paddedLength: options.nativeFileChunkPaddedLength || Math.max(NATIVE_AGENT_MESSAGE_PADDED_LENGTH, body.length + 512)
      });
      ratchet = encrypted.state;
      var agentEnvelope = encodeNativeAgentEnvelope({
        type: 'message',
        version: options.nativeAgentVersion || 7,
        encAgentMessage: encrypted.packet
      });
      var clientEnvelope = encodeNativeQueueClientMessage(outboundQueue, agentEnvelope, options);
      outboundQueue = clientEnvelope.queue;
      await this.client.sendQueueMessage(outboundQueue, clientEnvelope.body, {
        ...options,
        corrId: options.fileChunkCorrId ? String(options.fileChunkCorrId) + '-' + (i + 1) : ('file-chunk-' + (i + 1) + '-' + Date.now())
      });
    }
    this.store.saveQueue(fileQueueId + ':outbox', outboundQueue);
    this.store.save('nativeFile', fileQueueId, {
      ...fileState,
      accepted: true,
      completed: true,
    ...
    return {
      ...
      fileQueueId,
      sharedMsgId: fileState.sharedMsgId,
      accepted: true,
      completed: true,
      chunkCount: chunks.length
    };
