      ...
      elements.forEach(function (entry) {
        ...
        if (allowVotes && entryId) {
          voteControls = '<span class="list-entry-vote-controls" data-list-entry-id="' + escapeHtml(entryId) + '" aria-label="Entry score">' +
            '<button type="button" class="list-entry-vote-btn" data-list-public-action="vote" data-list-entry-id="' + escapeHtml(entryId) + '" data-list-vote-value="1" aria-label="Upvote" title="Upvote">' + voteArrow('up') + '</button>' +
            '<span class="list-entry-score" title="Score">' + escapeHtml(String(Number(entry && entry.list_score || 0) || 0)) + '</span>' +
            '<button type="button" class="list-entry-vote-btn" data-list-public-action="vote" data-list-entry-id="' + escapeHtml(entryId) + '" data-list-vote-value="-1" aria-label="Downvote" title="Downvote">' + voteArrow('down') + '</button>' +
          '</span>';
        }
        var rightMeta = '';
        if (signedIn) {
